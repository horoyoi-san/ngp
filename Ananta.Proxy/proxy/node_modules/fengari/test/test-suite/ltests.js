...
const getstring = function(L, buff, pc) {
    let i = 0;
    skip(pc);
    if (pc.script[pc.offset] === '"'.charCodeAt(0) || pc.script[pc.offset] === '\''.charCodeAt(0)) {  
...
const loadlib = function(L) {
    ...
    lauxlib.luaL_requiref(L1, to_luastring("package", true), null, 1);    
...
const Chook = function(L, ar) {
    ...
    scpt = lua.lua_tostring(L, -1);  
    lua.lua_pop(L, 2);  
    lua.lua_pushstring(L, events[ar.event]);  
    lua.lua_pushinteger(L, ar.currentline);  
    runJS(L, L, { script: scpt, offset: 0 });  
};
...
const panicback = function(L) {
    ...
    lua.lua_getfield(L, lua.LUA_REGISTRYINDEX, to_luastring("_jmpbuf", true));    
    b = lua.lua_touserdata(L, -1);
    lua.lua_pop(L, 1);    
    runJS(b.L, L, { script: b.paniccode, offset: 0 });    
    throw 1;
};
...
const checkpanic = function(L) {
    ...
    lua.lua_atpanic(L1, panicback);    
    lua.lua_pushlightuserdata(L1, b);
    lua.lua_setfield(L1, lua.LUA_REGISTRYINDEX, to_luastring("_jmpbuf", true));    
    try {    
        runJS(L, L1, { script: code, offset: 0 });    
...
const sethookaux = function(L, mask, count, scpt) {
    ...
    if (!lua.lua_istable(L, -1)) {  
        ...
        lua.lua_setfield(L, lua.LUA_REGISTRYINDEX, to_luastring("JS_HOOK", true));  
    }
    lua.lua_pushlightuserdata(L, L);
...
const buildop = function(p, pc) {
    ...
    let result = sprintf("(%4d) %4d - ", line, pc); 
    switch (lopcodes.getOpMode(o)) {
        case lopcodes.iABC:
            result += sprintf("%-12s%4d %4d %4d", name, lopcodes.GETARG_A(i), lopcodes.GETARG_B(i), lopcodes.GETARG_C(i)); 
            break;
        case lopcodes.iABx:
            result += sprintf("%-12s%4d %4d", name, lopcodes.GETARG_A(i), lopcodes.GETARG_Bx(i)); 
            break;
        case lopcodes.iAsBx:
            result += sprintf("%-12s%4d %4d", name, lopcodes.GETARG_A(i), lopcodes.GETARG_sBx(i)); 
            break;
        case lopcodes.iAx:
            result += sprintf("%-12s%4d", name, lopcodes.GETARG_Ax(i)); 
            break;
    }
