...
const db_getinfo = function(L) {
    ...
    lua_newtable(L);  
    if (luastring_indexOf(options, 83 ) > -1) {
    ...
    if (luastring_indexOf(options, 108 ) > -1)
        settabsi(L, to_luastring("currentline", true), ar.currentline);
    if (luastring_indexOf(options, 117 ) > -1) {
...
const makemask = function(smask, count) {
    let mask = 0;
    if (luastring_indexOf(smask, 99 ) > -1) mask |= LUA_MASKCALL;
    if (luastring_indexOf(smask, 114 ) > -1) mask |= LUA_MASKRET;
    if (luastring_indexOf(smask, 108 ) > -1) mask |= LUA_MASKLINE;
