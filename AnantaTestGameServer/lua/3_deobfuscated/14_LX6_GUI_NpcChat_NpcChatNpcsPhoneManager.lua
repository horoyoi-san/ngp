...
function M:GetPlayerSign()
	gNpcChatUtils.GetPlayerSignature(gPlayerManager.infoLogin.bindData.pid, function (sign)
...
function M:AddFakeChatChannelMessages(sub, isMind)
		...
		if cfg.NPCid == sub or cfg.ChatGroup == sub then
			while cfg do
				...
				if cfg.NextMessage and #cfg.NextMessage > 0 and cfg.NextMessage[1] ~= 0 then
					...
					if cfg.FakeChatAutoPlay and not isMind then
						cfg = nil
						gNpcChatManager.FakeChatAutoPlay = true
					end
				else
					cfg = nil
				end
...
function M:AddRestFakeChatChannelMessages(npcChatId)
...
function M:Reset()
	self:_InitData()
	gNpcChatManager:LoadChatChannelPlayerInfo()
end
...
function M:OpenFakePhoneChatPanel(params)
	...
	if chatCfg.ChatType == LTConfig.NPCChatConfig.ChatTypeType.Fake then
		gNpcChatUtils.OpenChatPanel(panelParams)
	elseif chatCfg.ChatType == LTConfig.NPCChatConfig.ChatTypeType.Dialog then
		self:TriggerDialogAndOpenChatPanel(chatCfg)
	else
		print_error("@zhangzhiyuan06 NPC 假手机还不支持的聊天类型！NPCChat 表 ID = " .. chatId .. ", 聊天类型 = " .. chatCfg.ChatType)
	end
end

function M:TriggerDialogAndOpenChatPanel(chatCfg)
	print_error("@zhangzhiyuan06 TriggerDialogAndOpenChatPanel NPCChat 表 ID = " .. chatCfg.Id)
end
...
function M:SetInstanceId2Args(instanceId, args)
	...
	if #self._instanceId2Args == cap then
		self._instanceId2Args[self._instanceId2Args_pos] = item
...
function M:OnPanelVisibleChange()
	if gPlayerManager.infoLogin.bindData.pid == nil or self.uncompletedFakeChat == nil then
		return
	end
...
function M.OnPanelShow(_, panelId)
	...
	if not gClientUtils.CheckIsMainPhonePanelId(panelId) then
		return
	end

	local data = gPanelManager.panelData[panelId]
...
gMessageManager:AddMessageListener(gEventConstants.PANEL_ON_SHOW, M.OnPanelShow)
...
function M:CheckShowChatPanel(data)
	...
	local store = gStoreManager:GetStoreGroup("NpcChatN2NChattingPanel")
	...
	local taskState = gTaskManager:GetTaskState(60004402)

	if self.showNpcFakeChatNextTime and taskState == UX.Game.TaskState.Accepted then
		print_error("60004402任务中打开 NPC 手机，忽略ShowNpcFakeChatNextTime，联系zhangzhiyuan06看看复现流程")
	end

	if self.showNpcFakeChatNextTime and taskState ~= UX.Game.TaskState.Accepted then
		local cfgId = self.showNpcFakeChatNextTime
		self.showNpcFakeChatNextTime = nil

		self:ShowNpcFakeChat(cfgId, instanceId)
	...
	if lastArgs then
		gNpcChatUtils.OpenChatPanel(lastArgs)
