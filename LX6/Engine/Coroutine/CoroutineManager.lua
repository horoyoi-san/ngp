-- Original chunk: @Lua\LuaFiles\LX6\Engine\Coroutine\CoroutineManager.lua
-- Decompiled from: 00138_CoroutineManager.lua_a74908ea9d69.luajit
...
M.OnUpdate = function(self)
	...
	for i = 1, #list do
		...
		if coInternal == nil and not coInternal.isCanceled then
			while true do
				local moveStatus = coInternal.TryMoveNext(coInternal)
...
M.StartCoroutine = function(self, coFunc, ...)
	local co = create(coFunc)
	local status, wait = resume(co, ...)
	local coInternal = Coroutine:New(co, wait, nil, )

	if not status then
		local msg = debug.traceback(co, wait)

		print_error(msg)
		coInternal.Finish(coInternal)

		return nil
	elseif not coInternal.HasNext(coInternal) then
		coInternal.Finish(coInternal)
...
M.StartCoroutineWithCallback = function(self, coFunc, ...)
	local arg = {
		...
	}
	local length = #arg
...
M.CancelCoroutine = function(self, coInternal)
