-- Original chunk: @Lua\LuaFiles\LX6\GUI\Chat\RandomChatManager.lua
-- Decompiled from: 00293_RandomChatManager.lua_fa677e586980.luajit
...
M.GetChatId = function(self, npcId, itemId)
	if self.itemId2Info[itemId] then
		return self.itemId2Info[itemId].chatId
	end

	local npcCfg = self.GetNpcCfg(self, npcId)
	local chatList = self.GetChatListByNpcCfg(self, npcCfg)
	local chatItem = self.GetRandomItem(self, chatList)
...
M.MakePhoneCfgByChatId = function(self, chatId)
	local npcsPhoneCfg = {
		["?@\\x90\\x9a\\xa1F"] = 0,
		["-7\\xb7\\xeb(z+\\xb72=Ȏ\\xb1:\\x98\\x9bƉ"] = false,
...
M.TriggerFakeChat = function(self, chatId)
...
M.GetSubChannelId = function(self, chatCfg)
	if chatCfg ~= nil then
...
M.IsMatchTag = function(self, npcTag, targetTags)
...
M.GetChatListByNpcCfg = function(self, npcCfg)
	local chatList = {}
	...
	for i = 0, count - 1 do
		...
		if self.IsMatchAllTags(self, npcCfg, chatSetCfg) then
...
M.InsertChatSetToList = function(self, chatList, chatSetCfg)
...
M.GetRandomItem = function(self, list)
	local totalWeight = 0
...
M.GetRandomImageId = function(self, config, itemId)
