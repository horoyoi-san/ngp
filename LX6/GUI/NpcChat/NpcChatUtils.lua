-- Original chunk: @Lua\LuaFiles\LX6\GUI\NpcChat\NpcChatUtils.lua
-- Decompiled from: 00307_NpcChatUtils.lua_4b0be3c2baf2.luajit
...
NpcChatUtils.OpenChatPanel = function(params)
...
NpcChatUtils.MakeInviteNpcView = function(npcTemplateId, npcChatInfo, npcCardInfo)
	local view = {
	...
	gNpcChatManager:SetHeadIcon(view, nil, , npcChatInfo:GetIcon())

	return view
end

NpcChatUtils.GetFavorLevel = function(favor)
...
NpcChatUtils.IsNextMessage = function(lastMsg, thisMsg)
...
NpcChatUtils.HasNextMessage = function(msg)
...
NpcChatUtils.IsFirstMessage = function(msg, topChannelId, subChannelId)
	local cfg = LTConfig.NPCChatConfig.GetConfig(msg.npcChatId)
	...
	if not cfg.IsFirst then
		return false
	end

	topChannelId = msg.topChannelId or topChannelId
	subChannelId = msg.subChannelId or subChannelId

	if not topChannelId or not subChannelId then
		return true
	end

	local channel = gNpcChatManager:GetChannel(topChannelId, subChannelId)
	...
	for i, channelMsg in ipairs(channel.messages) do
		if channelMsg.msgId ~= msg.msgId then
			currentIndex = i
	...
	if currentIndex <= 1 then
		local prevMsg = channel.messages[currentIndex - 1]

		if prevMsg and gNpcChatUtils.IsNextMessage(prevMsg, msg) then
			return false
		end
	end
...
NpcChatUtils.IsNextNpcMessage = function(msg)
	if msg.npcNextChatId <= 0 then
		return true
	end
...
NpcChatUtils.CheckIsValidChatCfgOnShowNpcNewChat = function(cfg)
	if cfg ~= nil then
		return false
	end
...
NpcChatUtils.SetSignature = function(topChannel, subChannel, setter)
...
NpcChatUtils.GetTextByTextId = function(textId)
	local textConfig = LTConfig.TextScriptTextConfig.GetConfig(textId)

	if textConfig and not string.is_null_or_empty(textConfig.Text) then
		return textConfig.Text
	end

	textConfig = LTConfig.TextConfig.GetConfig(textId)
...
NpcChatUtils.GetNpcGroupRenameTextId = function(groupId)
	local bindData = gPlayerManager.infoMinorNpcCultivation and gPlayerManager.infoMinorNpcCultivation.bindData
	local playerInfo = bindData and bindData.playerNpcCultivationInfo
	local renameDict = playerInfo and playerInfo.chatGroupRenameDict

	if renameDict ~= nil then
		return 0
	end

	return renameDict[groupId] or 0
end

NpcChatUtils.GetNpcGroupDisplayName = function(groupId, needPlainText)
	...
	local groupName = cfg.GroupName
	local renameTextId = this.GetNpcGroupRenameTextId(groupId)

	if renameTextId <= 0 then
		local renameText = this.GetTextByTextId(renameTextId)

		if not string.is_null_or_empty(renameText) then
			groupName = renameText
		end
	end

	if needPlainText then
		return gClientUtils.RichTextToPlain(groupName)
	end

	return groupName
end

NpcChatUtils.SetHeader = function(headerWidget, showMyInfo, topChannel, subChannel)
	...
	elseif topChannel ~= gNpcChatConst.ChatTopChannel.NpcGroup then
		headerStore.name = this.GetNpcGroupDisplayName(subChannel, true)
	else
		print_error("ChatUtils.SetHeader: invalid topChannel", topChannel)
	end
end

NpcChatUtils.IsNeedShowFrontFullscreen = function(params)
	...
	if params.chatCfg and params.chatCfg.SpecialReadableCharacter == 0 then
		return true
	end
...
NpcChatUtils.CheckParam = function(params, key, value, ...)
...
NpcChatUtils._OpenChatPanelParamsPreprocess = function(params)
	params = params or {}
...
NpcChatUtils._TryGetNpcsPhoneCfg = function(chatCfg)
	if not chatCfg then
		return nil
	end
...
NpcChatUtils.SortSubChannelItems = function(items)
...
NpcChatUtils.GetNpcChatChannelId = function(cfg)
	local isGroup = cfg.ChatGroup >= 0
...
NpcChatUtils.SetCloseType = function(...)
	local store = this.GetBasePanelStore()

	store.SetCloseType(store, ...)
end

NpcChatUtils.ShowPhoneAppTip = function(text)
	local store = this.GetBasePanelStore()

	store.ShowMessage(store, text)
end

NpcChatUtils.GetCurrentCloseType = function()
	local store = this.GetBasePanelStore()

	return store.closeType, store.customCloseFunc
end

NpcChatUtils.GetCurrentNpcChannelLastMsg = function(top, sub)
...
NpcChatUtils.GetBasePanelStore = function()
	return gStoreManager:GetStoreGroup("NpcChatBasePanelStore")
end

NpcChatUtils.IsChatPanelShowing = function()
...
NpcChatUtils.GetMainPhonePanelId = function()
...
NpcChatUtils.GetNextNpcChatId = function(msg)
	if msg.npcNextChatId <= 0 then
...
NpcChatUtils.GetFavorViewInfo = function(npcCultivationId)
...
NpcChatUtils.SetChatChannelCardBaseView = function(btn, topChannelId, subChannelId)
	if gClientUtils.IsNil(btn) then
		print_error("@zhangzhiyuan06 SetChatChannelCardBaseView btn is nil")
	...
	if store ~= nil then
		print_error("@zhangzhiyuan06 SetChatChannelCardBaseView btn.Store is nil!, btn", btn.name)
	end

	gNpcChatAvatarUtils:SetChannelAvatar(topChannelId, subChannelId, store.avatar)
end

NpcChatUtils.NotImplementedHint = function()
	gDisplayMessageMgr:ShowMessage(65401046)
end

NpcChatUtils.ShowSystemIcon = function(headRoot, icon)
	headRoot.headType = gNpcChatConst.HeadIconType.System
	headRoot.lihuiIcon = icon
end

NpcChatUtils.ShowNpcGroupIcons = function(headRoot, icons, groupLimit)
...
NpcChatUtils.ShowPZHeadInfoHead = function(headRoot, headInfo, isMe)
	local type, path = gImageManager:GetHeadIconByHeadIconInfo(headInfo, gPlayerManager.infoLogin.bindData.sexType, isMe)
	local iconId = path
	local cfg = LTConfig.ImageNewAvatarConfig.GetConfig(iconId)
	...
	gNpcChatUtils.ShowSystemIcon(headRoot, cfg.SguiImageId)
end

NpcChatUtils.TurnChatCfgListToOptions = function(nextChatCfgList)
	local options = {}
...
NpcChatUtils.IsNextPlayerEllipsis = function(chatId)
...
NpcChatUtils.NewFromNpcChatItem = function(chatItem, isSkipAllMessage)
	...
	msg.isSkipAllMessage = isSkipAllMessage or false

	return msg
end

NpcChatUtils.NewFromNpcChatMsg = function(msg)
	if msg ~= nil then
		return nil
	end
...
NpcChatUtils.ConvertCSNpcChatData = function(npcChatData)
	local data = {
...
NpcChatUtils.ConvertCSNpcGroupChatData = function(npcGroupChatData)
	local data = {
...
NpcChatUtils.GetChatNotifyListKey = function(showDataList)
	if not showDataList or #showDataList ~= 0 then
		return ""
	end
	...
	for _, data in ipairs(showDataList) do
		table.insert(keys, tostring(data.subChannelId))
	end
...
NpcChatUtils.CheckAndMarkPopupPending = function(popupKey)
	if not popupKey or popupKey ~= "" then
		return false
	end

	if gNpcChatManager.pendingPopupKeys[popupKey] then
		return false
	end

	gNpcChatManager.pendingPopupKeys[popupKey] = true

	return true
end

NpcChatUtils.MarkPopupRendered = function(popupKey)
	if popupKey and popupKey == "" then
		gNpcChatManager.pendingPopupKeys[popupKey] = nil
	end
end

NpcChatUtils.ShowChatNotify = function(topChannel, subChannel)
	...
	if not gNpcChatUtils.ShouldShowInOnlineMode(channel.lastMessage.cfg) then
		return
	end
	...
	local popupKey = tostring(subChannel)

	if not gNpcChatUtils.CheckAndMarkPopupPending(popupKey) then
		return
	end

	param.popupKey = popupKey
...
NpcChatUtils.ShowChatNotifyList = function(showDataList)
	if not showDataList or #showDataList ~= 0 then
		return
	end

	if #showDataList ~= 1 then
		gNpcChatUtils.ShowChatNotify(showDataList[1].topChannelId, showDataList[1].subChannelId)
	elseif #showDataList <= 1 then
		local popupKey = gNpcChatUtils.GetChatNotifyListKey(showDataList)
...
NpcChatUtils.IsAuxiliaryChannel = function(topChannelId, auxiliaryChannelId)
...
NpcChatUtils.GetCurrentNpcId = function()
...
NpcChatUtils.IsVisibleToCurrentNpc = function(chatId)
...
NpcChatUtils.ShouldShowInOnlineMode = function(cfg)
	if not cfg then
		return true
	end

	if gLinkManager:CheckInLinkMode() and not cfg.IsOnline then
		return false
	end
...
NpcChatUtils.CanShowInNormalChatHistory = function(msg, curNpc)
...
NpcChatUtils.GetNormalAndOtherToSelfMessages = function(channelInfo)
...
NpcChatUtils.IsCurrentInChatPage = function()
...
NpcChatUtils.CheckSwitchTimelineEnd = function()
	return not LX6.Scene.SwitchSceneManager.AfterSwitchShow_CheckShowPlaying() and not gLoadingManager:SwitchTeleport_CheckIsLoading()
end

NpcChatUtils.GetMessage = function(cfg)
	if not cfg then
		return ""
	end
...
NpcChatUtils.HaveNormalTypeChat = function(topChannelId, subChannelId)
...
NpcChatUtils.GetUncompletedChatAndDialogCount = function(npcId)
...
NpcChatUtils._CountOneNpcUncompletedChatsForNpc = function(chatData, targetNpcId)
	local count = 0
		...
		if chatInfo and chatInfo.ChatList and #chatInfo.ChatList <= 0 then
			...
			if #dialogMessages <= 0 then
				...
				if lastMsg and gNpcChatUtils.HasNextMessage(lastMsg) then
					...
					if cfg and gNpcChatUtils.IsVisibleToNpc(cfg, targetNpcId) and gNpcChatUtils.ShouldShowInOnlineMode(cfg) then
						count = count + 1
					end
				end
			end
...
NpcChatUtils.IsVisibleToNpc = function(cfg, targetNpcId)
...
NpcChatUtils.GetTaskControlValue = function(eventId)
...
NpcChatUtils.ShouldAskReadChat = function(cfg)
	if not cfg then
		return false
	end

	if gNpcChatManager.isSceneLoading then
		return false
	end
...
NpcChatUtils.ResumeChatAutoClick = function()
	local chatPanel = gStoreManager:GetStoreGroup("NpcChatChattingPanelStore")
	local active1 = chatPanel and chatPanel.rootWidget and chatPanel.rootWidget.gameObject.activeInHierarchy
	local chatN2NPanel = gStoreManager:GetStoreGroup("NpcChatN2NChattingPanel")
	local active2 = chatN2NPanel and chatN2NPanel.rootWidget and chatN2NPanel.rootWidget.gameObject.activeInHierarchy
	...
	if active1 then
		if chatPanel.lastMessage and chatPanel.STATE_EnableOnce and chatPanel.bActive and chatPanel.rootWidget.activation and gNpcChatUtils.HasNextMessage(chatPanel.lastMessage) and (not chatPanel.optionList or not chatPanel.optionListData or #chatPanel.optionListData ~= 0) then
			chatPanel.DoAutoClick(chatPanel, chatPanel.lastMessage)

			return
		end

		chatPanel.OnClickChatBG(chatPanel)
	elseif active2 then
		chatN2NPanel.OnClickChatBG(chatN2NPanel)
	end
end
