-- Original chunk: @Lua\LuaFiles\LX6\GUI\Season\SeasonRaidUtils.lua
-- Decompiled from: 02173_SeasonRaidUtils.lua_40347b2be1c8.luajit
...
local SeasonRaidUtils = {
	...
	SetChaosProgress = function (self, bindData, current, max)
		if max ~= nil then
			max = self.GetMaxChaosValue(self)
		end
	...
	SetupCardData = function (self, itemId, cell)
		if self.IsChaosBuff(self, itemId) then
			self.SetupCardType(self, cell, SeasonItemType.buff)
		...
		elseif self.IsChaosItem(self, itemId) then
			self.SetupCardType(self, cell, SeasonItemType.qiwu)
	...
	GenerateCardTemplateData = function (self, itemId)
		local cell = {}
		local itemType = self.SetupCardData(self, itemId, cell)
