-- Original chunk: @Lua\LuaFiles\LX6\GUI\Npc\NpcInteracsUtils.lua
-- Decompiled from: 00350_NpcInteracsUtils.lua_61144f411901.luajit

local NpcCultivationVoiceConfig = LTConfig.NpcCultivationVoiceConfig
local M = {
	MSG_TYPE = {
		["IW\\x8fRs\\x86\\xddxL[]i"] = 2,
		["8m\\xa5\\xaf\\xaam"] = 1,
		["WSn"] = 3,
		["T\rS~"] = 0
	},
...
M.CreateNpcCardInfo = function(self, info, npcCultivationId)
	local cardInfo = {
...
M.CheckIsUnlocked = function(self, npcCardId)
...
M.CheckIfCanInteract = function(self, npcCardId)
	if not self.CheckIsUnlocked(self, npcCardId) then
...
M.TryGetNpcCultivationInfo = function(self, npcCardId)
	if self.CheckIsUnlocked(self, npcCardId) then
		local index = gPlayerManager.infoMinorNpcCultivation.bindData.npcCultivationInfosDic[npcCardId]
		local favorInfo = gPlayerManager.infoMinorNpcCultivation.bindData.npcCultivationInfos[index]
...
M.RemoveUnlockNpcCultivationInfo = function(self, npcCardId)
	local index = gPlayerManager.infoMinorNpcCultivation.bindData.unlockedNpcCultivationInfosDic[npcCardId]
	...
	self.RefreshUnlockedNpcCultivationInfosDic(self)
end

M.RefreshUnlockedNpcCultivationInfosDic = function(self)
...
M.GetInteractableNpcSet = function(self)
