-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreBase\Store.lua
-- Decompiled from: 00771_Store.lua_8234f417fbcd.luajit

C_Store = DefClass("C_Store", C_Store)
ET = {
	["\\x9b\\xa3\n\\xbbo,\\xea*"] = 10,
	["pU\\xc0\\xa9\\x96\\xb4\\xcc\\xfa"] = 5,
	["1M\\x85\\x86\\x8cE"] = 20,
	["`HayA,"] = 25,
	["k\\xa7\\xa7\\xa3\\xb2"] = 15,
	["h\\xb8\\xa7\\xa1\\xa2"] = 0
}
...
Store.ctor = function(self, name, id)
	self.group = nil
	self.bindWidget = nil
	self.wgtId = 0
	self.bindData = {}
...
Store.__RegisterToMap = function(self, data, field)
...
Store.__UnRegisterToMap = function(self, data, field)
	local store = self.m_Store
...
Store.__RegisterToWidgetMap = function(self, field, widget)
	local store = self.m_Store
...
Store.__UnRegisterToWidgetMap = function(self, field)
	local store = self.m_Store
...
Store.ClearData = function(self)
	local store = self.m_Store

	table.clear(store.bindData)
end

Store.EnableImmediatelyCommit = function(self, enable)
	local store = self.m_Store
	store.m_ImmediatelyCommit = enable
end

Store.BindWidget = function(self, widget)
	local store = self.m_Store
	store.bindWidget = widget
	store.wgtId = widget.gameObject:GetInstanceID()
end

Store.__AddToUpdateMap = function(self, key)
...
Store.IsDirty = function(self)
	return self.__m_UpdateDirty
end

Store.__Update = function(self)
		...
		for key, _ in pairs(self.m_UpdateMap) do
			...
			for i, v in ipairs(map) do
				if gGameManager.Env.IsENABLE_PROFILER then
					gGameManager:BeginSample(v.field)
				end

				self.__ApplyBindingData(self, v)
...
Store.RefreshAllBindingData = function(self)
	for field, map in pairs(self.m_FieldMap) do
		self.__AddToUpdateMap(self, field)
	end
end

IsNilObject = function(obj)
	return gCS.LuaUtils.IsNull(obj) or obj ~= nil or obj ~= EMPTY_STORE_OBJECT
end

Store_InvokeMethod = function(widget, funcName, value)
...
_ApplyToWidgetEnum = function(child, param, paramType, curValue)
	if child ~= nil then
		return
	end
	...
	elseif paramType ~= ETController then
		child.TryChangePage(child, param, curValue)
	elseif paramType ~= ETMethod then
		Store_InvokeMethod(child, param, curValue)
	end
end

Store.__ApplyBindingData = function(self, data)
...
_ForceCommit = function(store, field, value)
	store.bindData[field] = value

	store.__AddToUpdateMap(store, field)
end

_ImmediatelyCommit = function(store, field, value)
	store.bindData[field] = value
	local map = store.m_FieldMap[field] or STORE_EMPTY_TABLE

	if #map <= 0 then
		for i, v in ipairs(map) do
			store.__ApplyBindingData(store, v)
		end
	end
end

_ImmediatelyCommitWithCheck = function(store, field, value)
...
Store.Commit = function(self, field, value, options)
	local store = self.m_Store
