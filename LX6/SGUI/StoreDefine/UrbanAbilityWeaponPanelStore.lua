-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\UrbanAbilityWeaponPanelStore.lua
-- Decompiled from: 01137_UrbanAbilityWeaponPanelStore.lua_b210a49edf7c.luajit
...
M.OnAwake = function(self)
	...
	self.bindData.attributeList.luaSimpleRenderItem = self:CreateAction("OnRenderAttributeItem")
	...
	self.bindData.skillList.luaSimpleRenderItem = self:CreateAction("OnRenderSkillItem")

	self.bindData.attributeList.onGetTIndex = function(csIndex)
		return self._attributeListData[csIndex + 1].tIndex
	end

	self.bindData.skillList.onGetTIndex = function(csIndex)
		return self._skillListData[csIndex + 1].tIndex
	end
...
M.SetAttrs = function(self)
	...
	self._attributeListData = list

	self.bindData.attributeList:SetSimpleList(#list)
end
...
M.OnRenderWeaponItem = function(self, btn, index)
	...
	if self.weaponData[index + 1] then
		...
		local cfg = LTConfig.SceneitemConfig.GetConfig(id)
		...
		if index ~= 1 then
			self.OnWeaponClick(self, data)
		end

		store.button.luaClick = self.CreateActionWithArgs(self, "OnWeaponClick", data)
	else
		store.icon = 0
...
M.OnRenderAttributeItem = function(self, btn, index)
	local data = self._attributeListData[index + 1]
...
M.OnWeaponClick = function(self, data)
...
M.SetRightData = function(self, data)
	...
	self.SetSkillList(self, data.cfg)
end

M.SetSkillList = function(self, cfg)
	local skillList = {}

	if cfg.CommonSkill then
		table.insert(skillList, {
			["\\xe6Y-\\xdc\\x9fB\\xaeX\\x99\\xb2"] = 28004195,
	...
	if cfg.HeavyCommonSkill then
		table.insert(skillList, {
			["\\xe6Y-\\xdc\\x9fB\\xaeX\\x99\\xb2"] = 28000923,
	...
	if cfg.ActiveSkill then
		table.insert(skillList, {
			["\\xe6Y-\\xdc\\x9fB\\xaeX\\x99\\xb2"] = 28000889,
	...
	if cfg.UniqueSkill then
		table.insert(skillList, {
			["\\xe6Y-\\xdc\\x9fB\\xaeX\\x99\\xb2"] = 28004193,
	...
	if cfg.DodgeSkill then
		table.insert(skillList, {
			["\\xe6Y-\\xdc\\x9fB\\xaeX\\x99\\xb2"] = 28001009,
	...
	self._skillListData = list

	self.bindData.skillList:SetSimpleList(#list)
end

M.OnRenderSkillItem = function(self, btn, index)
	local data = self._skillListData[index + 1]
	...
	store.button.luaClick = self.CreateActionWithArgs(self, "OnSkillClick", args)
end

M.OnSkillClick = function(self, args)
