-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\RadarPuzzlePanelStore.lua
-- Decompiled from: 00896_RadarPuzzlePanelStore.lua_80b2ee43f7dd.luajit
...
local AnimName = {
	["\\xacaa"] = "\\xb8\\x9c\\xa8\\xf9\\xea\\xc1\\x99\\xf2\\xfd\\xcfV\\xe2Џ\\xa2\t0\\xbb\\x93\\xff\\xd1ܦ\\xf7",
	["\\xa3ab"] = "\\xb8\\x9c\\xa8\\xf9\\xea\\xc1\\x99\\xf2\\xfd\\xcfV\\xe2Џ\\xa2\t0\\xbb\t\\x93\\xfc\\xd1ܦ\\xf7",
	["~\\xa3\\xa3\\xa3\\xba"] = "b\\xda\\x89\\xdc\\xe8X\\xb2\\xb5'\\xa3pE\\xc0\\xfe3=a\\xce\\xdbx\\xb0\n\\xb8>x\\xcd'\\xaa"
}
...
M.OnActiveDeviceChanged = function(self, eventId, device)
...
M.OnShow = function(self, panelId, data)
	...
	for i = 1, #self.cfgs.Answer do
		...
		if self.Answer[i] == 0 then
			self.SetGray(self, self.bindData["p" .. i], i)
		end
		...
		for j = 1, 3 do
			...
			self.SetSinglePuzzle(self, self.bindData["p" .. i], 0, self.Answer[i], false)
...
M.SetGray = function(self, go, index)
	local id = go.gameObject:GetInstanceID()
	local store = self:GetStoreById(id)
	store.bg = 1
end

M.SetSinglePuzzle = function(self, go, color, num, isPlayAnim)
	local id = go.gameObject:GetInstanceID()
	local store = self:GetStoreById(id)
...
M.ShowDragRing = function(self, cfgs, r)
...
M.RegisterEvents = function(self)
	self.bindData.ButtonR.luaClick = self.CreateAction(self, "RotateRight")
	self.bindData.ButtonL.luaClick = self.CreateAction(self, "RotateLeft")
	self.bindData.ButtonSwitch.luaClick = self.CreateAction(self, "SwitchSelect")
	self.bindData.ButtonClose.luaClick = self.CreateAction(self, "ClickClose")
	self.bindData.ButtonCancel.luaClick = self.CreateAction(self, "OnCancel")

	for i = 1, 4 do
		self.bindData["ring" .. i].luaClick = self.CreateActionWithArgs(self, "OptionPlayStationClick", i)
		self.bindData["ring" .. i].luaBeginDrag = self.CreateActionWithArgs(self, "OptionDragStart", i)
		self.bindData["ring" .. i].luaDrag = self.CreateActionWithArgs(self, "OptionDrag", i)
		self.bindData["ring" .. i].luaEndDrag = self.CreateActionWithArgs(self, "OptionDragEnd", i)
	end

	for i = 1, 3 do
		self.bindData["r" .. i].luaClick = self.CreateActionWithArgs(self, "SignalClick", i)
		self.bindData["r" .. i].luaBeginDrag = self.CreateActionWithArgs(self, "SignalDragStart", i)
		self.bindData["r" .. i].luaDrag = self.CreateActionWithArgs(self, "SignalDrag", i)
		self.bindData["r" .. i].luaEndDrag = self.CreateActionWithArgs(self, "SignalDragEnd", i)
		self.bindData["r" .. i].luaHover = self.CreateActionWithArgs(self, "OnHover", i)
		self.bindData["r" .. i].luaUnhover = self.CreateActionWithArgs(self, "OnUnHover", i)
	end
end
...
M.OnUnHover = function(self, index)
	if self.isDragRight then
		return
	end
...
M.SignalClick = function(self, index)
	if not self.occupiedRing[index] then
		return
	end

	if self.BanOperate(self) then
		return
	end
...
M.OnCancel = function(self)
	...
	if not self.occupiedRing[self.curSelect] or not self.curSelect then
		...
		self.RemoveRing(self, self.curSelect)
...
M.BanDrag = function(self, index, enable)
	for i = 1, 3 do
		if i == index then
			if enable then
...
M.SignalDragStart = function(self, index)
	if self.BanOperate(self) then
		return
	end
...
M.SignalDrag = function(self, index)
	if not self.curSelect then
		return
	end
...
M.SignalDragEnd = function(self, index)
	if self.BanOperate(self) then
		return
	end
	...
	self.BanDrag(self, self.curSelect, true)

	if self.InFingerRange(self, self.curSelect, 1) then
		for i = 1, 16 do
			if i > 1 and i < 9 then
				self.bindData["r" .. self.curSelect].gameObject.transform:Find("ring/ring0" .. i).gameObject:SetActive(false)
			else
...
M.OptionPlayStationClick = function(self, index)
...
M.OptionDragStart = function(self, index)
	if self.BanOperate(self) then
		return
	end
...
M.OptionDrag = function(self, index)
	if self.BanOperate(self) then
		return
	end
end

M.OptionDragEnd = function(self, index)
	if self.BanOperate(self) then
		return
	end

	if self.InFingerRange(self, index, 2) then
		self.bindData["ring" .. index].gameObject:FindChild("ring").gameObject:SetActive(false)
...
M.AddRing = function(self, index)
	if not self.gamepadMode and self.SignalIsFull(self) then
		self.RemoveRing(self, self.curSelect)
	end
	...
	elseif not lastIndex and self.curSelect then
		...
		newIndex = self.GetEmptyIndex(self)
	end
	...
	self.SetSelect(self, newIndex)
	self.UpdateValue(self, values, index)
	self.RefreshResult(self)
end
...
M.UpdateValue = function(self, values, index)
	for i = 1, 16 do
...
M.ControlRLBtn = function(self, isEnable)
	if isEnable then
...
M.RotateRight = function(self)
	...
	self.StartRotate(self, 1)
end

M.RotateLeft = function(self)
	if self.BanOperate(self) then
		return
	end
...
M.StartRotate = function(self, dir)
	timeCount = 0
...
M.SwitchSelect = function(self)
	if self.BanOperate(self) then
		return
	end

	self.OnClickSwitch(self)
end

M.BanOperate = function(self)
	return self.isWin or self.startRotate
end

M.OnClickSwitch = function(self)
	if not self.curSelect then
		self.SetSelect(self, 1)
	end
...
M.RemoveRing = function(self, curSelect)
...
M.InFingerRange = function(self, index, pos)
	if pos ~= 1 then
