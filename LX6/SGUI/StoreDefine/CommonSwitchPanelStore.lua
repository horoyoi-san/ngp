-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\CommonSwitchPanelStore.lua
-- Decompiled from: 01537_CommonSwitchPanelStore.lua_4caed1b25201.luajit
...
M.OnAwake = function(self)
	self.bindData.carList.luaSimpleRenderItem = self.CreateActionWithArgs(self, self.OnRenderContentItem, StaticProps.Tabs.CAR)
	self.bindData.characterList.luaSimpleRenderItem = self.CreateActionWithArgs(self, self.OnRenderContentItem, StaticProps.Tabs.AVATAR)
	self.bindData.onlinePoseList.luaSimpleRenderItem = self.CreateActionWithArgs(self, self.OnRenderContentItem, StaticProps.Tabs.ONLINE_POSE)
	self.bindData.carList.luaSelectedChanged = self.CreateActionWithArgs(self, self.OnSelectedChange, StaticProps.Tabs.CAR)
	self.bindData.characterList.luaSelectedChanged = self.CreateActionWithArgs(self, self.OnSelectedChange, StaticProps.Tabs.AVATAR)
	self.bindData.onlinePoseList.luaSelectedChanged = self.CreateActionWithArgs(self, self.OnSelectedChange, StaticProps.Tabs.ONLINE_POSE)
	self.bindData.fashionBtn.luaClick = self.CreateAction(self, self.OnClickFashionBtn)
	self.bindData.modBtn.luaClick = self.CreateAction(self, self.OnClickModBtn)
end
...
M.OnTabSelectedChanged = function(self, uList)
	...
	if self.bindData.type ~= StaticProps.Tabs.AVATAR then
		self.RefreshAvatarList(self)
	elseif self.bindData.type ~= StaticProps.Tabs.CAR then
		self.RefreshCarList(self)
	end
end

M.OnRenderContentItem = function(self, tabIndex, btn, index)
...
M.OnSelectedChange = function(self, tabIndex, list)
...
M.SetData = function(self, enableVehicle, selectDict, callback)
...
M.OnBodyTypeRefresh = function(self, bodyType)
	...
	if self.bindData.type ~= StaticProps.Tabs.ONLINE_POSE then
		self.RefreshOnlinePoseList(self)
	end
end

M.RefreshPage = function(self, tabList)
...
M.RefreshAvatarList = function(self)
...
M.RefreshOnlinePoseList = function(self)
...
M.RefreshCarList = function(self)
	if not self.getUnlockedVehicleDone then
		self.RequestUnlockedVehicle(self)

		return
	end
end

M.RequestUnlockedVehicle = function(self)
	slot1 = gDriveVehiclesManager

	slot1:GetUnlockedVehicleInfo(function (vehicleList)
		...
		local currentVehicleLimit = gLinkManager.currentGameCfg and gLinkManager.currentGameCfg.VehicleTypeLimit
		...
		if currentVehicleLimit then
			for i = 1, #vehicleList do
				...
				if vehicleCfg then
					if array.contains(currentVehicleLimit, vehicleCfg.VehicleType) then
						local ele = {
...
M.OnClickFashionBtn = function(self)
