-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\MotoControlsDriverStore.lua
-- Decompiled from: 01992_MotoControlsDriverStore.lua_fa2ae8079664.luajit

local VehicleType = LTConfig.VehicleConfig.VehicleTypeType
local HudDescConfig = LTConfig.HudDescConfig
C_MotoControlsDriverStore = DefClass("C_MotoControlsDriverStore", C_MotoControlsDriverStore, C_DriverStoreBase)
GroupName2Class.MotoControlsDriverStore = C_MotoControlsDriverStore
local M = C_MotoControlsDriverStore
...
M.OnGroupEnable = function(self)
	...
	self.forwardBtnStore.btnId = HudDescConfig.MOTO_FORWARD_BTN
	self.backwardBtnStore.btnId = HudDescConfig.MOTO_BACKWARD_BTN
	self.driftBtnStore.btnId = HudDescConfig.MOTO_DRIFT_BTN
	self.driftBtnJoystickStore.btnId = HudDescConfig.MOTO_DRIFT_JOYSTICK_BTN
...
M.OnEnterFinish = function(self)
	...
	self.isTri = cfg and (cfg.VehicleType ~= VehicleType.Tricycle or cfg.VehicleType ~= VehicleType.Ebike)
	self.bindData.isTricycleCtrl = self.isTri and self.ControlType.True or self.ControlType.False
...
M.RegisterWidget = function(self)
	self.bindData.upBtn.luaPress = self.CreateActionWithArgs(self, "OnClickUpBtn", true)
	self.bindData.upBtn.luaRelease = self.CreateActionWithArgs(self, "OnClickUpBtn", false)
	self.bindData.downBtn.luaPress = self.CreateActionWithArgs(self, "OnClickDownBtn", true)
	self.bindData.downBtn.luaRelease = self.CreateActionWithArgs(self, "OnClickDownBtn", false)
...
M.OnClickDownBtn = function(self, isDown)
	self.downPress = isDown

	if self.vehicleCs then
		self.vehicleCs:MotorcycleRearWheelieButtonHandle(isDown)
	end
end

M.OnClickUpBtn = function(self, isDown)
	self.upPress = isDown

	if self.vehicleCs then
		self.vehicleCs:MotorcycleFrontWheelieButtonHandle(isDown)
	end
end
...
M.OnLeftStickControl = function(self, context)
	if (context.started or context.performed) and self.vehicleCs then
		...
		self.vehicleCs:MotorcycleSteerAndWheelieGamepadHandle(val.x, self.isTri and 0 or val.y)
	end
