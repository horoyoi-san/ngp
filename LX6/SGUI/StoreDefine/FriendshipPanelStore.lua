-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\FriendshipPanelStore.lua
-- Decompiled from: 01852_FriendshipPanelStore.lua_01467b477861.luajit
...
M.ctor = function(self)
	...
	self.ANIME_TYPE = {
		["UXu"] = "^\\xc1]\\xfb\\xdf\\xe0f9\\xfalV\\xf3\\xf9h\\xcde|\\xd3\\xdc\\xc0Ҍ\\xf7j",
		["59"] = "\\x9d3\\xb7\\xbc\\xea\\xe7ݭ\\xf4\\xef\\xdcJ\\xd7\\xee\\xb2\\xa23\\xaa2\\x9f\\xf4\\xdb\\xfe\\xa8\\xff",
		["\\xafLB"] = "\\xa2!\\xe1D&O\\x9cdF\\x92xG}#\\x80f\\xf9\\xb6\\xae2\\xd2Je\\xbb\n",
		["n\\x82\\x8d\\x9c\\x93"] = "0ٱ=/\\xe8O+\\xbe\\x817H\\xb4O\\x8de.\\xbd<\\xe5\\x8b*\\xcb"
	}
...
M.ProcessData = function(self, param)
...
M.InitAnime = function(self)
	...
	self.PlayAnimation(self, self.ANIME_TYPE.ADD)
	self.PlayRootAnimation(self, self.ANIME_TYPE.OPEN)
end
...
M.OnUpdate = function(self)
	if self.needUpdateNormal then
		if self.finishTime < self.updateTime then
			...
			self.PlayRootAnimation(self, self.ANIME_TYPE.CLOSE)
		...
		elseif self.updateTime >= self.phase2 then
			if not self.switchAnimePhase1 then
				self.PlayAnimation(self, self.ANIME_TYPE.UP)
...
M.PlayAnimation = function(self, anime)
	gCS.LuaUtils.PlayAnimationByName(self.bindData.anime, anime)
end

M.PlayRootAnimation = function(self, anime)
