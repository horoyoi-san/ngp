-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\GameplayPressBarStore.lua
-- Decompiled from: 01761_GameplayPressBarStore.lua_082fa9241f3a.luajit
...
M.OnShow = function(self, panelId, data)
	...
	self.minValue = showData.minValue
	self.maxValue = showData.maxValue
	self.maxTime = showData.maxTime
	self.randomShake = showData.randomShake
	self.shakeTriggerValue = showData.shakeTriggerValue
	self.shakeValue = showData.shakeValue
	self.shakeStrength = showData.shakeStrength
	self.pressed = false
	self.pressing = false
	self.upSpeed = 1 / showData.maxTime
	self.value = 0
	self.randomDir = -1
...
M.OnReleaseBtn = function(self)
	if self.pressed and self.pressing then
		...
		gMessageManager:SendMessage(gEventConstants.CAR_PRESS_BAR_STATE_CHANGE, {
			["LWig~4"] = false
		})
	end
end
