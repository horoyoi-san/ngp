-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\Police\PoliceSearchPanelStore.lua
-- Decompiled from: 01236_PoliceSearchPanelStore.lua_a8c6c2b611a2.luajit
...
M.OnAwake = function(self)
	...
	self.bindData.exitButton2.luaClick = self.CreateAction(self, self.Close)
	self.bindData.iconList.luaSimpleRenderItem = self.CreateAction(self, self.OnRenderIconItem)
	self.bindData.textList.luaSimpleRenderItem = self.CreateAction(self, self.OnRenderTextItem)
	self.bindData.textList.luaSimpleClick = self.CreateAction(self, self.OnTextItemClick)
	self.panelId = nil
	self.anim = self.rootWidget.anim
end

M.DelayShowRootWidget = function(self)
...
M.ShowSearchCarResult = function(self)
	local itemList = {}
	...
	self.ShowContentIconList(self, itemList, cfg)
end

M.SetAdditionalCfg = function(self, additionalCfg)
	self.bindData.text1 = additionalCfg.Text1
	self.bindData.text2 = additionalCfg.Text2
end

M.ShowContentIconList = function(self, content, additionalCfg)
	self.SetAdditionalCfg(self, additionalCfg)
	self.PlayAnim(self, "vx_S_PoliceSearchPanel_Item")
...
M.ShowSearchBodyContent = function(self, module, additionalCfg)
	local itemList = {}
...
M.ShowBreathCheckContent = function(self, module, additionalCfg)
	local isDrunk = false
	local fineId = 0
...
M.ShowDrugTestContent = function(self, module, additionalCfg)
	local drugList = {}
...
M.ShowContentTextList = function(self, content, additionalCfg, suggestion)
	if #content ~= 0 then
	...
	self.SetAdditionalCfg(self, additionalCfg)
	self.PlayAnim(self, "vx_S_PoliceSearchPanel_Alc")
...
M.OnRenderTextItem = function(self, btn, index)
...
M.OnTextItemClick = function(self, btn, index)
	local data = self.textData[index + 1]
...
M.StartAnimFailWatchdog = function(self)
