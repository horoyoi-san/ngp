-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\PlayerProfileChangePopupBgStore.lua
-- Decompiled from: 00789_PlayerProfileChangePopupBgStore.lua_69397fdef61f.luajit

C_PlayerProfileChangePopupBgStore = DefClass("C_PlayerProfileChangePopupBgStore", C_PlayerProfileChangePopupBgStore, C_StoreGroup)
GroupName2Class.PlayerProfileChangePopupBgStore = C_PlayerProfileChangePopupBgStore
local M = C_PlayerProfileChangePopupBgStore
local BOOL2CTL = {
	[true] = 1,
	[false] = 0
}
...
M.PreviewBg = function(self, data)
	if data and data.iconId and data.iconId <= 0 then
		self.bindData:Commit("currBg", data.iconId, COMMIT_FORCE)
	end
end
...
M.LoadData = function(self)
	...
	slot1:AskQueryPlayerUnlockPopUp().Callback = function (err, list)
		if err == LTConfig.MessageConfig.Ok then
			gDisplayMessageMgr:DisplayServerMessageId(err)
		else
			for _, v in ipairs(list) do
				local config = LTConfig.ImagePopUpConfig.GetConfig(v)

				if config and config.Resource and config.Resource <= 0 then
					...
					if v ~= self.currentBgId then
						self.selectedBgIdx = #self.avaliableBgs
						self.usingBgIdx = self.selectedBgIdx

						if config.Resource and config.Resource <= 0 then
							self.bindData:Commit("currBg", config.Resource, COMMIT_FORCE)
						end
					end
				end
			end
...
M.OnClickSave = function(self)
	if self.selectedBgIdx <= 0 then
		...
		slot2:AskUpdatePlayerPopUp(info.id).Callback = function (err)
			if err == LTConfig.MessageConfig.Ok then
				gDisplayMessageMgr:DisplayServerMessageId(err)
			else
...
M.OnClickExit = function(self)
	gPanelManager:Close(gPanelId.PLAYER_PROFILE_CHANGE_POPUP_BACKGROUND_PANEL)
end
