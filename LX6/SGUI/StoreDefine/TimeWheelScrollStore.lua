-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\TimeWheelScrollStore.lua
-- Decompiled from: 01367_TimeWheelScrollStore.lua_f044c1faa341.luajit
...
M.StartWheel = function(self, startTime, endTime, duration)
	self.InitModel(self, startTime, endTime, duration)
	self.InitView(self)
end

M.InitModel = function(self, startTime, endTime, duration)
...
M.InitView = function(self)
	...
	self.SetGirdPositionByTime(self, startHour, startMinute)
end

M.SetGirdPositionByTime = function(self, startHour, startMinute)
...
M.MoveNode = function(self, transform, count, targetNum, speed)
	if targetNum ~= 0 then
		targetNum = count
	end
...
M.OnUpdate = function(self)
	if self.isStart then
		...
		if h0 == self.lastH0 then
			self.lastH0 = self.MoveNode(self, self.bindData.h0Transform, 4, h0, self.h0RotateSpeed)
		end
		...
		if h1 == self.lastH1 then
			...
			self.lastH1 = self.MoveNode(self, self.bindData.h1Transform, count, h1, self.h1RotateSpeed)
		end

		if m0 == self.lastM0 then
			self.lastM0 = self.MoveNode(self, self.bindData.m0Transform, 7, m0, self.m0RotateSpeed)
		end

		if m1 == self.lastM1 then
			self.lastM1 = self.MoveNode(self, self.bindData.m1Transform, 10, m1, self.m1RotateSpeed)
		end
