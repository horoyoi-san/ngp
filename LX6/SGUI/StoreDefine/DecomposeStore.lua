-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\DecomposeStore.lua
-- Decompiled from: 01668_DecomposeStore.lua_dab9a7de3015.luajit
...
M.OnAwake = function(self)
	...
	self.bindData.itemList.luaSimpleClick = self.CreateAction(self, self.OnItemListBtnClick)
	self.bindData.itemList.luaSelectedChanged = self.CreateAction(self, self.OnItemListSelectedChange)
	...
	self.bindData.decomposeList.luaSimpleRenderItem = self.CreateAction(self, "OnRenderDecomposeListItem")

	self.bindData.decomposeList.onGetTIndex = function(index)
	...
	self.bindData.decomposeBtn.luaClick = self.CreateAction(self, "OnDecomposeBtnClick")
...
M.OnRenderItem = function(self, btn, index)
	...
	if data.itemId then
		store = gCommonItemManager:OnCommonItemRender(btn, index, self.breakList[data.id])
	end

	if store and store.vBtn then
		store.vBtn.luaClick = function()
...
M.OnRenderDecomposeListItem = function(self, btn, index)
	local data = self.decomposeListData[index + 1]
...
M.RefreshItemList = function(self)
	...
	slot4 = self.bindData.itemList

	slot4:SetSimpleList(#self.itemListData)
...
M.OnItemListBtnClick = function(self, btn, index, fromVBtn)
	local data = self.itemListData[index + 1]
	...
	self.ChangeSelectListCount(self, data, count)
...
M.OnDecomposeBtnClick = function(self)
...
M.OnRewardChange = function(self)
	local rewardDict = {}
	self.decomposeListData = {}
	...
	table.sort(self.decomposeListData, self.CreateAction(self, "SortRenderItem", gCommonItemManager))
...
M.OnBuyNumChange = function(self, num)
	self.ChangeSelectListCount(self, self.targetItem, num)
end

M.ChangeSelectListCount = function(self, data, count)
