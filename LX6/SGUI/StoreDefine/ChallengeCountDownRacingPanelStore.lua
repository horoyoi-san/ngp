-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\ChallengeCountDownRacingPanelStore.lua
-- Decompiled from: 01647_ChallengeCountDownRacingPanelStore.lua_f5ba282d37d9.luajit

C_ChallengeCountDownRacingPanelStore = DefClass("C_ChallengeCountDownRacingPanelStore", C_ChallengeCountDownRacingPanelStore, C_StoreGroup)
GroupName2Class.ChallengeCountDownRacingPanelStore = C_ChallengeCountDownRacingPanelStore
local M = C_ChallengeCountDownRacingPanelStore
...
M.OnAwake = function(self)
	...
	self.timeRange = 0
	slot1 = gLinkManager
	self.inLink = slot1:CheckInLinkMode()
	self.msgEvents = {
...
M.SetDurationTime = function(self, data)
	if data.startTime then
		local durationTime = gCS.TimeManager.ServerUnixTime - data.startTime
		self.startTime = self.startTime - durationTime
		self.endTime = self.endTime - durationTime
	end
end

M.RefreshCountDown = function(self)
	if self.isMatchGameCountDown and self.reverseCountDown then
		self.bindData.countDownTime = self.GetFormatCountDownTime(self, self.nowTime + self.matchGameCountDown)
	else
...
M.CheckMatchGameCountDown = function(self, isMatchGameCountDown)
	self.isMatchGameCountDown = false
	self.matchGameCountDown = 0

	if isMatchGameCountDown then
		local matchGameCountDown = self.GetMatchGameCountDown(self)
...
M.GetMatchGameCountDown = function(self)
