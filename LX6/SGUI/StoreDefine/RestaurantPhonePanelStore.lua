-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\RestaurantPhonePanelStore.lua
-- Decompiled from: 00906_RestaurantPhonePanelStore.lua_90d7dfb43986.luajit
...
M.OnAwake = function(self)
	self.bindData.menuList.luaSimpleRenderItem = self.CreateAction(self, "OnRenderMenuItem")
	self.bindData.menuList.luaSimpleClick = self.CreateAction(self, "OnClickMenuItem")
	self.bindData.btnBack.luaClick = self.CreateAction(self, "OnClickBtnBack")
	self.bindData.btnCommit.luaClick = self.CreateAction(self, "OnClickBtnCommit")
	self.bindData.btnDetailCommit.luaClick = self.CreateAction(self, "OnClickBtnDetailCommit")
	self.cfg = nil
	self.focusNpc = nil
	self.mode = nil
	self.gameplayId = nil
...
M.OnShow = function(self, panelId, data)
	...
	if data.disableLeaveOrder then
		self.gameplayHudPanelStore:SetBtnBackState(false)
	end
...
M.InitFoodMenu = function(self)
	self.menuList = {}
	local menu = gRestaurantManager:GetDateMenu() or self.cfg.Foodincluded

	for _, foodId in ipairs(menu) do
...
M.SetCameraFocusTarget = function(self, csUnit, mode)
...
M.RefreshMoneyView = function(self)
	...
	local templateId = gCommonItemManager:GetItemIdByMoneyType(UX.Game.MoneyType.Money)
	...
	moneyShowStore.count = gCommonItemManager:BuildLargeNum(money)
	moneyShowStore.moneyText = consumableCfg.MoneyRichTextIcon
	moneyShowStore.iconButton.luaRenderTooltip = self:CreateActionWithArgs("OnRenderToolTips", {
		TemplateId = templateId
	}, gCommonItemManager)
end

M.CheckSpecialFoodOrder = function(self, foodId)
	local menuList = self.GetCurrentMenu(self)
...
M.GetCurrentMenu = function(self)
	return self.menuList
end

M.AddOrder = function(self, index)
	...
	if not self.orderList[index] then
		...
		self.RefreshOrderCountAndCost(self)
		self.RefreshBtnStyle(self)
		self.CheckShowAtomsphereReaction(self, true, index)
	end
end

M.RemoveOrder = function(self, index, refreshBtn, refreshDialog)
	...
	if self.orderList[index] then
		...
		if refreshDialog then
			self.CheckShowAtomsphereReaction(self, false, index)
		end
	end
end

M.ClearAllOrder = function(self)
	if not self.orderList then
		return
	end

	for index, _ in pairs(self.orderList) do
		self.RemoveOrder(self, index)
	end
end

M.RefreshOrderCountAndCost = function(self)
	local exchangeCost = gCommonItemManager:GetExchangeRate(self.totalCost)
	self.bindData.totalCostText = string.format("%s %d", gCommonItemManager:GetCurrMoneyRichText(), exchangeCost)
...
M.SwitchTab = function(self, tabIndex)
	if tabIndex ~= 0 then
		...
		self.RefreshFoodDetailTab(self)
...
M.RefreshFoodDetailTab = function(self)
	local food = self.menuList[self.curFoodDetailIndex]
	local exchangePrice = gCommonItemManager:GetExchangeRate(food.price)
	self.bindData.curFoodIconUrl = gUIUtils:GetSguiImagePath(food.foodIconId)
	self.bindData.curFoodBuffIconUrl = gUIUtils:GetSguiImagePath(food.effectIconId)
	...
	self.bindData.curFoodPriceText = string.format("%s %d", gCommonItemManager:GetCurrMoneyRichText(), exchangePrice)
	self.bindData.curFoodBtnStateCtrl = food.order and 1 or 0
end

M.CheckShowAtomsphereReaction = function(self, isAdd, index)
	return
	...
	if C_RestaurantManager.ORDER_MODE.ALONE >= self.mode and self.npcDailyCfg and self.OrderTextCD >= Time.time - self.lastAtomsphereDialogTime then
		...
		if isAdd then
			...
			for i = 1, #self.npcDailyCfg.OrderSpecialSet do
				...
				if foodId ~= OrderSpecialSet.foodid1 and self.CheckSpecialFoodOrder(self, OrderSpecialSet.foodid2) and OrderSpecialSet.dialogid then
			...
			for i = 1, #self.npcDailyCfg.OrderSpecialSet do
				...
				if foodId ~= OrderSpecialSet.foodid2 and self.CheckSpecialFoodOrder(self, OrderSpecialSet.foodid1) and OrderSpecialSet.dialogid then
					table.insert(reactions, OrderSpecialSet)
				end
			end
		...
		if #reactions <= 0 then
			...
			if reaction.actionid == 0 then
				gClientUtils.PlaySingleAction(self.focusNpc, reaction.actionid, reaction.groupid, 0, 0.5)
			end
...
M.OnRenderMenuItem = function(self, btn, index)
	...
	local exchangePrice = gCommonItemManager:GetExchangeRate(data.price)
	local store = self:GetStoreById(id)

	if store then
		store.foodNameText = data.foodName
		store.foodPriceText = string.format("%s %d", gCommonItemManager:GetCurrMoneyRichText(), exchangePrice)
		store.moneyIconId = gCommonItemManager:GetMoneyImageConfigIdByType(UX.Game.MoneyType.Money)
		store.foodIconUrl = gUIUtils:GetSguiImagePath(data.foodIconId)
		store.foodBuffIconUrl = gUIUtils:GetSguiImagePath(data.effectIconId)
...
M.OnClickMenuItem = function(self, btn, index)
	...
	if store then
		...
		self.SwitchTab(self, 1)
	end
end

M.OnClickMenuItemBtn = function(self, data)
	local store = self.GetStoreById(self, data.id)

	if store then
		local food = self.menuList[data.index]

		if food.order then
			self.RemoveOrder(self, data.index, true, true)
		else
			self.AddOrder(self, data.index)
		end
...
M.OnClickBtnSwitchCamera = function(self)
...
M.OnClickBtnBack = function(self)
	self.SwitchTab(self, 0)
end

M.OnClickBtnCommit = function(self)
	...
	local buyFoodInfo = {
		RestaurantId = gRestaurantManager.dateReplaceMenuId or self.cfg.Id,
		FoodIdList = {}
	}
...
M.OnClickBtnDetailCommit = function(self)
	local food = self.menuList[self.curFoodDetailIndex]

	if food.order then
		self.RemoveOrder(self, self.curFoodDetailIndex, true, true)
	else
		self.AddOrder(self, self.curFoodDetailIndex)
	end
