-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\BrawlerNeedleStore.lua
-- Decompiled from: 01625_BrawlerNeedleStore.lua_8da365ff9a70.luajit

C_BrawlerNeedleStore = DefClass("C_BrawlerNeedleStore", C_BrawlerNeedleStore, C_StoreGroup)
GroupName2Class.BrawlerNeedleStore = C_BrawlerNeedleStore
local M = C_BrawlerNeedleStore
...
M.OnClose = function(self)
	...
	if self.acupointHideTimer then
		self.acupointHideTimer:Stop()

		self.acupointHideTimer = nil
	end
	...
	gFlyNeedleGameManager:DestroyGame()
end
...
M.ShowAcupointResult = function(self, text)
	if text ~= nil or text ~= "" then
		return
	end

	self.bindData.tipText = text
	self.bindData.resultTextCtrl = 2

	if self.acupointHideTimer then
		self.acupointHideTimer:Stop()
	end

	self.acupointHideTimer = Timer.New(function ()
		self.acupointHideTimer = nil
		self.bindData.resultTextCtrl = 0
	end, 3)

	self.acupointHideTimer:Start()
end
...
M.OnShoot = function(self)
	...
	gFlyNeedleLog.Log(gFlyNeedleLog.Cat.Shoot, "OnShoot 射线", "missLevel=" .. tostring(self.missLevel), "hasHitInfo=" .. tostring(hitInfo == nil), "hasCollider=" .. tostring(hitInfo == nil and hitInfo.collider == nil), "hitGO=" .. tostring(hitGameObject and hitGameObject.name))
