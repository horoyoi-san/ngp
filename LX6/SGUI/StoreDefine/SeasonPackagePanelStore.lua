-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\SeasonPackagePanelStore.lua
-- Decompiled from: 00880_SeasonPackagePanelStore.lua_5225149bc20b.luajit
...
M.RegisterButtons = function(self)
	...
	self.bindData.leftTabBtn.luaClick = self.CreateActionWithArgs(self, "OnTabBtnClick", -1)
	self.bindData.rightTabBtn.luaClick = self.CreateActionWithArgs(self, "OnTabBtnClick", 1)
end
...
M.OnTabBtnClick = function(self, direction)
...
M.RegisterLists = function(self)
	self.bindData.typeList.luaRenderItem = self.CreateAction(self, "OnRenderTypeList")
	self.bindData.buffList.luaRenderItem = self.CreateAction(self, "OnRenderBuffList")
	self.bindData.qiwuList.luaRenderItem = self.CreateAction(self, "OnRenderQiwuList")
	self.bindData.typeList.luaClick = self.CreateAction(self, "OnClickTypeList")
	self.bindData.buffList.luaClick = self.CreateAction(self, "OnClickBuffList")
	self.bindData.qiwuList.luaClick = self.CreateAction(self, "OnClickQiwuList")
end

M.OnRenderTypeList = function(self, btn, index, data)
...
M.OnClickTypeList = function(self, btn, data)
...
M.OnRenderQiwuList = function(self, btn, index, data)
	local id = btn.gameObject:GetInstanceID()
...
M.OnClickQiwuList = function(self, btn, data)
	if not self.toolTipStore then
		return
	end
...
M.RefreshTab = function(self, data)
