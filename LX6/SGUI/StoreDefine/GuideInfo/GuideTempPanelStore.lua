-- Original chunk: @Lua\LuaFiles\LX6\SGUI\StoreDefine\GuideInfo\GuideTempPanelStore.lua
-- Decompiled from: 01205_GuideTempPanelStore.lua_c687e6445343.luajit
...
M.OnShow = function(self, panelId, params)
	self.panelId = panelId
	self.params = params

	if table.isNilOrEmpty(params) then
		self.params = {
			["`f\\xa9yI\\xb6\\xd0^^{mG"] = true
		}
	end
...
M.InitView = function(self)
	self.SetupStartPage(self, self.params)
	self.InitTabBtnList(self)
end

M.SetupStartPage = function(self, params)
...
M.InitTabBtnList = function(self)
	self.btnListData = {}
...
M.OnRenderTab = function(self, csIndex, widget)
...
M.OnRenderTabButton = function(self, btn, csIndex)
...
M.OnTabClick = function(self, btn, data)
...
M.ClickToNextTab = function(self)
	...
	if nextIndex >= self.tabCount then
		...
		self.OnTabClick(self, tabBtnList.items[nextIndex], tabBtnList.GetData(tabBtnList, nextIndex))
	end
end
