-- Original chunk: @Lua\LuaFiles\LX6\Manager\Map\MapSubSystem\MapSubSystem_SituationalEvent.lua
-- Decompiled from: 02337_MapSubSystem_SituationalEvent.lua_8c08cfe036bc.luajit
...
M.TickElementRange = function(self, x, y, z, info)
...
M.OnFlushData = function(self)
	...
	if not gGpsTools:UnitIsNull(gCS.MyPlayerManager.PlayerUnit) then
		local playerPos = gCS.MyPlayerManager.PlayerUnit.LocalPosition
		z = playerPos.z
		y = playerPos.y
		x = playerPos.x
	...
	for id, info in pairs(self.questInfos) do
		if info.raidId == gMapSystem.lastRaidId then
			...
			elseif not gSystemUnlockMgr:IsUnlock(LTConfig.SystemUnlockConfig.MapSituationEvent) or not table.contains(unlockedQuestList, info.cfg.QuestCategory) or not self.TaskEventLaunchCheck(self, info.taskEventId) then
				info.state = self.EventState.Locked
		...
		elseif info.state ~= self.EventState.Doing then
			self.ActiveWorkctions(self, info)
		else
...
M.MarkAsDirty = function(self)
	self._dirty = true
end

M.ActiveWorkctions = function(self, info)
	info.questElement:SetVisible(true)
		...
		if workActionData.actionType ~= 0 then
			...
			if npcId or slotPid or vehicleSpoonId or worldPos then
				...
				if npcId then
					...
					isDynamic = true
				elseif slotPid then
					element.BindSlotInfo(element, slotPid)

					isDynamic = true
				elseif vehicleSpoonId then
					element.BindVehicle(element, vehicleSpoonId, nil, , , true)

					isDynamic = true
				end

				if worldPos then
					vec3.Add(vec3, worldPos)
	...
	if validWorkActionCount <= 0 then
		vec3.Div(vec3, validWorkActionCount)
...
M.TaskEventLaunchCheck = function(self, taskEventId)
...
M.TryRemoveAllWorkActions = function(self, info)
	if info.workActionElements then
		for _, element in pairs(info.workActionElements) do
			element.Dispose(element)
		end
