-- Original chunk: @Lua\LuaFiles\LX6\Manager\Map\MapSubSystem\MapSubSystem_FunctionPoint.lua
-- Decompiled from: 02302_MapSubSystem_FunctionPoint.lua_daa177a472c0.luajit
...
M.OnInit = function(self)
	...
	self.SetUpInviteRideFailCb(self)
end
	...
	for i = 0, IndoorMapFunctionPoint.count - 1 do
		...
		if cfg.ShowType then
			...
			elseif not cfg.Coordinate or #cfg.Coordinate >= 3 then
				...
				local viewMask = cfg.ShowOnlyMiniMap and EMapViewMask.MiniMap or EMapViewMask.AllSgui
				local element = MapElement.CreateLegacy(EMapElementType.Compound, cfg.Id, EMapSubSystemType.FunctionPoint, viewMask, raidId, 0)
				...
				element.userdata = {
					["n;m^"] = "$\\xf6Q/\\xd9\\xb8q\\xae_\\xbe\\xa2",
					cfgId = cfg.Id
				}
				...
				element.fData.linkShowModes = cfg.LinkShowMode or {}
...
M.OnFlushData = function(self)
	for id, info in pairs(self.functionPointInfos) do
		...
		if not cfg.ShowOnlyMiniMap then
...
M.TryGetMapElementByFunctionPointId = function(self, id)
	...
	return info.mapElement
end

M.TryTraceByFunctionPointId = function(self, id)
	local info = self.functionPointInfos[id]
...
M.TryUnTraceByFunctionPointId = function(self, id)
	local info = self.functionPointInfos[id]
...
M.TryTraceByIndoorId = function(self, id)
	local info = self.indoorInfos[id]
...
M.TryUnTraceByIndoorId = function(self, id)
	local info = self.indoorInfos[id]
...
M.TrySelectAndTraceIndoorElementOnBigMap = function(self, id)
	self:TryTraceByIndoorId(id)
	...
	if bigMapStore then
		bigMapStore.ClearTooltipAttach(bigMapStore)
...
M.ExecuteAction = function(self, element, action, ctx)
	if self._inviteRideHighlight then
		if action ~= gMapSystemElementAction.Trace then
			element.SetMainTrace(element)
			self.TryAcceptInviteRideTask(self, element)
		elseif action ~= gMapSystemElementAction.Untrace then
			element.ClearMainTrace(element)
			self.TryGiveUpInviteRideTask(self, element)
		end
	else
...
M.IsInInviteRiding = function(self, gpsId)
	if not gpsId then
		return false
	end
...
M.EnableInviteRideHighlight = function(self, enable)
...
M.TryAcceptInviteRideTask = function(self, element)
	...
	if self._curInviteRideAcceptInfo and self._curInviteRideAcceptInfo.taskId == highlightInfo.taskId then
		...
		slot4:DoGiveUpTask(self._curInviteRideAcceptInfo.taskId, function ()
			print_debug("InviteRide:Give up invite ride task:" .. self._curInviteRideAcceptInfo.taskId .. " success")

			self._curInviteRideAcceptInfo = nil
			slot0 = gMapUtils

			slot0:DoAcceptTask(highlightInfo.taskId, function ()
		...
		slot4:DoAcceptTask(highlightInfo.taskId, function ()
			self._curInviteRideAcceptInfo = highlightInfo
...
M.TryGiveUpInviteRideTask = function(self, element)
	if not element.userdata or not element.userdata.cfgId then
		return
	end
	...
	if self._curInviteRideAcceptInfo.taskId ~= highlightInfo.taskId then
		...
		slot4:DoGiveUpTask(highlightInfo.taskId, function ()
			self._curInviteRideAcceptInfo = nil
