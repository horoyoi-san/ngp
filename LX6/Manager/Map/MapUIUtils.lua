-- Original chunk: @Lua\LuaFiles\LX6\Manager\Map\MapUIUtils.lua
-- Decompiled from: 00185_MapUIUtils.lua_8a834d4f1fc7.luajit

gMapUIUtils = gMapUIUtils or {}
local M = gMapUIUtils

M.GetMapConfig = function(raidId, indoorId)
	local mapCfg = {
		scaleData = {}
	}
	...
	if indoorId <= 0 then
		...
		if indoorCfg.ExtraMapResInfo and #indoorCfg.ExtraMapResInfo <= 0 then
		...
		if sceneCfg.ExtraMapResInfo and #sceneCfg.ExtraMapResInfo <= 0 then
			mapCfg.extraUnifiedMapInfo = sceneCfg.ExtraMapResInfo
		end
...
M.HasConfig = function(raidId, indoorId)
	if indoorId <= 0 then
		local indoorCfg = LTConfig.IndoorConfig.GetConfig(indoorId)
...
M.GetElementActionName = function(action)
