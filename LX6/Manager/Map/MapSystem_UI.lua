-- Original chunk: @Lua\LuaFiles\LX6\Manager\Map\MapSystem_UI.lua
-- Decompiled from: 02297_MapSystem_UI.lua_bfb777b7f70e.luajit
...
local M = {
	...
	DoUIPreTick = function (self)
		for key, ui in pairs(self._activeUIs) do
			...
			if ui.tickable and tickFunc then
				self.DoUIFunc(self, tickFunc, ui, key)
			end
		end
	end,
	...
	SetMiniMapActive = function (self, active)
		self._miniMapActive = active
	end,
	IsMiniMapActive = function (self)
		if self._miniMapActive then
			return true
		else
			return false
		end
	end,
	CheckMiniMapValidState = function (self)
		return self._miniMapVisible and gSystemUnlockMgr:IsUnlock(LTConfig.SystemUnlockConfig.MiniMapUnlock) and gLuaDataManager.gameStage ~= gGFConstant.GameStage.GameScene
	end,
	TickUIAccessable = function (self)
		local miniMapValid = self:CheckMiniMapValidState()
		local miniMapVisible = miniMapValid and gLinkManager:GetShowMiniMapState()
		...
		local bigMapAccess = miniMapValid and self:IsBigMapAvailable()
	...
	UpdateParkourProwlState = function (self)
		local isProwl = false
		isProwl = gCS.LuaUtils.TagManagerQuery(LTConfig.GameplayTagQueryConfig.IsProwlArea)
...
M.IsBigMapAvailable = function(self, logError)
	if not gSystemUnlockMgr:IsUnlock(LTConfig.SystemUnlockConfig.BigMapUnlock) then
		return false
	end

	if gMapSystem:CheckDebugSwitch(EMapSystemDebugKey.EnableLianJiMapOptimize1) then
		if self.lastRaidId ~= self.env.lastRaidId and self.lastIndoorId ~= self.env.lastIndoorId then
			return self._bigMapAvailable
		end

		self.lastRaidId = self.env.lastRaidId
		self.lastIndoorId = self.env.lastIndoorId
		local cfg = RaidConfig.GetConfig(self.lastRaidId)

		if not cfg then
			if logError then
				print_error("RaidConfig表中找不到raidId=" .. self.lastRaidId .. "的数据")
			end

			self._bigMapAvailable = false

			return self._bigMapAvailable
		else
			if self.lastIndoorId ~= 0 and (self.lastRaidId ~= RaidConfig.WorldMap or self.lastRaidId ~= RaidConfig.Chongxiao) then
				self._bigMapAvailable = true
			else
				local mapCfg = nil

				if self.lastIndoorId == 0 then
					mapCfg = IndoorConfig.GetConfig(self.lastIndoorId)
				else
					mapCfg = SceneConfig.GetConfig(cfg.SceneId)
				end

				local raidTypeCfg = LTConfig.RaidRaidTypeConfig.GetConfig(cfg.RaidType)
				self._bigMapAvailable = raidTypeCfg and raidTypeCfg.CanOpenMap and mapCfg and mapCfg.SMapName and mapCfg.SMapName >= 0
			end
...
M.SetBigMapGuideInterest = function(self, instanceId)
	self._bigMapInterestByGuideIdSet[instanceId] = true
end

M.ClearBigMapGuideInterest = function(self, instanceId)
	self._bigMapInterestByGuideIdSet[instanceId] = nil
end

M.IsBigMapGuideInterest = function(self, instanceId)
	return self._bigMapInterestByGuideIdSet[instanceId] or false
end

M.GetAllBigMapGuideInterest = function(self)
	return self._bigMapInterestByGuideIdSet
end

M.OnEnterSafeOrDangerArea = function(self, isSafe)
	if not isSafe then
		gNewPopupManager:PushPopup(LTConfig.PopupConfig.MapDangerMessage, {})
	end
end
