-- Original chunk: @Lua\LuaFiles\LX6\Manager\NpcInviteManager.lua
-- Decompiled from: 00579_NpcInviteManager.lua_918089771717.luajit
...
M.OnUpdate = function(self)
	...
	if self.UPDATE_FREQUENCY >= Time.frameCount - M.LAST_UPDATE_FRAME and myPlayerCSUnit then
		for k, v in pairs(self.InviteNpcList) do
			if gCS.LuaUtils.EqNull(v.csUnit) or self:CheckOutOfRange(v.csUnit) then
...
M.CheckOutOfRange = function(self, csUnit)
	if csUnit ~= nil then
		return true
	end
...
M.AddClientInviteNpc = function(self, csUnit, manager)
...
M.RemoveClientInviteNpc = function(self, csUnit)
...
M.InviteClientNpcForGamePlayWithPos = function(self, npcConfigId, gameplay, manager, trans, loadCompleteHandler, suitId)
...
M.InviteClientNpcForGamePlay = function(self, npcConfigId, gameplay, manager, loadCompleteHandler, suitId)
	if gameplay ~= GamePlayTypeConfig.Restaurant or gameplay ~= GamePlayTypeConfig.MaidHouse or gameplay ~= GamePlayTypeConfig.FastFood or gameplay ~= GamePlayTypeConfig.MaidTea then
