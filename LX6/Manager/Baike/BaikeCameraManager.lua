-- Original chunk: @Lua\LuaFiles\LX6\Manager\Baike\BaikeCameraManager.lua
-- Decompiled from: 01562_BaikeCameraManager.lua_88dae6790b80.luajit
...
local GetCameraConfigByCameraType = function(cameraType)
	local config = {}
	...
	elseif cameraType ~= M.CameraType.Mall then
		config.rotateSpeed = CityPediaConfig.FashionCameraRotateSpeed
		config.mouseOffsetDPI = CityPediaConfig.FashionChangeCameraOffsetMouseDPI
		config.rotateMouseDPI = CityPediaConfig.FashionCameraRotateMouseDPI
		config.zoomSpeed = CityPediaConfig.FashionCameraZoomSpeed
...
M.SetBaikePanelCamera = function(self, panelId, enable, params)
	if not enable then
...
M.UpdateBaikePanelCamera = function(self)
	if #panelStack < 0 then
		...
		self:InitCamera(params.verticalButton, params.basePanel, params.rightStickCustomNavRespond, params.L2CustomNavRespond, params.R2CustomNavRespond, params.camera, params.modelRoot, params.cameraOffsetRange, params.banRotate, params.cameraType, params.cameraOffset, params.cameraEuler, params.fov, params.checkButtonEnabled, params.cameraControlConfig)
		self:RegisterEvent()
	end
end
...
M.InitCamera = function(self, verticalButton, basePanel, rightStickCustomNavRespond, L2CustomNavRespond, R2CustomNavRespond, camera, modelRoot, cameraOffsetRange, banRotate, cameraType, cameraOffset, cameraEuler, fov, checkButtonEnabled, cameraControlConfig)
	...
	if cameraControlConfig then
		cameraConfig.rotateSpeed = cameraControlConfig.rotateSpeed or cameraConfig.rotateSpeed
		cameraConfig.mouseOffsetDPI = cameraControlConfig.moveMouseDPI or cameraConfig.mouseOffsetDPI
		cameraConfig.rotateMouseDPI = cameraControlConfig.rotateMouseDPI or cameraConfig.rotateMouseDPI
		cameraConfig.zoomSpeed = cameraControlConfig.zoomSpeed or cameraConfig.zoomSpeed
		cameraConfig.zoomRange = cameraControlConfig.zoomRange or cameraConfig.zoomRange

		if cameraOffsetRange ~= nil and cameraControlConfig.yOffsetRange then
	...
	self.camera = camera
	self.cameraType = cameraType or M.CameraType.Fashion
	self.modelRoot = modelRoot
	self.banMove = false
	self.countFinish = false
	self.CameraRotateSpeed = cameraConfig.rotateSpeed or 1
	self.mouseOffsetDPI = cameraConfig.mouseOffsetDPI or 1
	self.rotateMouseDPI = cameraConfig.rotateMouseDPI or 1
	self.zoomSpeed = cameraConfig.zoomSpeed or 0.1
	self.zoomRange = cameraConfig.zoomRange or {
		-10,
		-1
	}
	...
	self.checkButtonEnabled = checkButtonEnabled
	self.targetCameraPos = nil
	self.zoomLerpSpeed = 10

	if not gCS.LuaUtils.IsNull(camera) then
...
M.ChangeCameraOffset = function(self, x, y)
...
M.ZoomCamera = function(self, zoomValue)
	...
	local worldPointCamera = camera

	if self.cameraType ~= M.CameraType.Mall and gCS.CameraDataMgr and gCS.CameraDataMgr.MainCamera then
		worldPointCamera = gCS.CameraDataMgr.MainCamera
	end

	if gCS.LuaUtils.IsNull(worldPointCamera) then
		worldPointCamera = camera
	end
	...
	local worldPointBefore = worldPointCamera:ScreenToWorldPoint(screenPoint)
	...
	local worldPointAfter = worldPointCamera:ScreenToWorldPoint(screenPoint)
