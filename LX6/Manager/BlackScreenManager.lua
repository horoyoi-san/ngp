-- Original chunk: @Lua\LuaFiles\LX6\Manager\BlackScreenManager.lua
-- Decompiled from: 00580_BlackScreenManager.lua_9583ff1b3efc.luajit
...
M.ctor = function(self)
	...
	self.blackPanel = nil
	self.debug = false
	self.isBlackTL = false
	self.isBlackLoading = false
	self.isBlackEdge = false
	self.setId = nil
	self.text = ""
	self.isWhite = false
	...
	self.delayCloseTimer = gMessageManager:AddMessageListener(gEventConstants.PANEL_ON_SHOW, function (eventId, data)
		if data ~= gPanelId.PVP_LOADING_PANEL and self.crossScene and self.blackPanel and self.isTransition then
...
M.OpenTransition = function(self, id, text, isWhite, crossScene, openTime, stayTimeMin, stayTimeMax, closeTime, openCb, stayCb, closeCb)
...
M.CloseTransition = function(self, id, closeTime, stayCb, closeCb)
	if not self.blackPanel then
		if self.debug then
...
M.AutoTransition = function(self, id, text, isWhite, crossScene, openTime, stayTime, closeTime, openCb, stayCb, closeCb)
	if not self.blackPanel then
		if self.debug then
...
M.ClearTransition = function(self, id, force)
	if not self.blackPanel then
...
M.IsOccupiedById = function(self, id)
	return self.isTransition and self.setId ~= id
end

M.IsOccupied = function(self)
	return self.isTransition
end

M.OpenBlackInstantly = function(self, id)
	if self.debug then
...
M.OpenBlackTL = function(self, isWhite)
	if not self.blackPanel or self.isBlackTL then
		return
	end

	self.isBlackTL = true

	self.blackPanel:OpenBlackTL(isWhite)

	if self.debug then
		print_notice("BLACK_SCREEN_MANAGER => OpenBlackTL", Time.time, Time.frameCount)
	end
end

M.UpdateBlackTL = function(self, isWhite, alpha)
	if not self.blackPanel or not self.isBlackTL then
		return
	end

	self.blackPanel:UpdateBlackTL(isWhite, alpha)
end

M.CloseBlackTL = function(self)
	if not self.isBlackTL then
		return
	end
	...
	self.blackPanel:CloseBlackTL()

	if self.debug then
		print_notice("BLACK_SCREEN_MANAGER => CloseBlackTL", Time.time, Time.frameCount)
	end
end

M.OpenBlackLoading = function(self)
	if not self.blackPanel or self.isBlackLoading then
		return
	end

	self.isBlackLoading = true

	self.blackPanel:OpenBlackLoading()
end

M.CloseBlackLoading = function(self)
	if not self.blackPanel or not self.isBlackLoading then
		return
	end

	self.isBlackLoading = false

	self.blackPanel:CloseBlackLoading()
end

M.OpenBlackEdge = function(self, isWhite)
	if not self.blackPanel or self.isBlackEdge then
		return
	end

	self.isBlackEdge = true

	self.blackPanel:OpenBlackEdge(isWhite)
end

M.UpdateBlackEdge = function(self, isWhite, designW, designH, ScreenW, ScreenH, lerpValue)
	if not self.blackPanel or not self.isBlackEdge then
		return
	end

	self.blackPanel:UpdateBlackEdge(isWhite, designW, designH, ScreenW, ScreenH, lerpValue)
end

M.CloseBlackEdge = function(self)
	if not self.isBlackEdge then
		return
	end
	...
	self.blackPanel:CloseBlackEdge()
end

M.CheckIdMatch = function(self, id)
	local match = false
...
M.OnTransitionEnd = function(self)
	self.isTransition = false
	self.transitionStartTime = -1
...
M.SetTransition = function(self, id, text, isWhite, crossScene, openTime, stayTimeMin, stayTimeMax, closeTime, openCb, stayCb, closeCb)
	self:ClearDelayClose()
...
M.Record = function(self, id, text, isWhite, crossScene, openTime, stayTimeMin, stayTimeMax, closeTime, openCb, stayCb, closeCb)
	self.setId = id
	self.text = text or ""
...
M.ClearDelayClose = function(self)
...
M.SetDebugMode = function(self, debug)
