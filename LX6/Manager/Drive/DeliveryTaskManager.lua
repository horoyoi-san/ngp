-- Original chunk: @Lua\LuaFiles\LX6\Manager\Drive\DeliveryTaskManager.lua
-- Decompiled from: 00688_DeliveryTaskManager.lua_1fe6cccc3931.luajit
...
M.ctor = function(self)
	...
	self.TitleType = {
		["5\\xe2V8%\\xc2\\xb3S\\x95S\\xa8\\xa2"] = 1,
		["##\\xefF\\x88\\xd8&\\xac*\\xf4\\xc6\\xe3l\\xef"] = 2,
		["\n\\xeaX$%\\xc2\\xb3S\\x95S\\xa8\\xa2"] = 5,
		["1\\xe6Q(%\\xc2\\xb3S\\x95S\\xa8\\xa2"] = 4,
		[".\\xecL)9\\xc4\\xa2D\\x95S\\xa8\\xa2"] = 3
	}
	...
	self.CargoState = {
		["\\x85\\xbe5\\xa2i5\\xcb#"] = 0,
		["1\\xe6M%\\xc5\\x92@\\xacW\\xb7\\xb3"] = 4,
		["5F\\x85\\x8f\\x80U"] = 1,
		["V-n^"] = 5,
		[":A\\x9f\\x87\\x90I"] = 6,
		["ʕ\\xe9\\xf2Ή\\xe0\\x83'-"] = 3,
		["\\z\\xa5pD\\xa6\\xd6Fg{yI"] = 2
	}
	self.curTitleType = 1
	self.msgEvents = {
		...
		[gEventConstants.L50_BEFORE_SWITCH_SCENE] = function (_, data)
		...
		[gEventConstants.L50_AFTER_SWITCH_SCENE] = function (_, switchSceneEventParams)
			...
			self:OnAfterSwitchScene(switchType)
		end,
		[gEventConstants.DRONE_DELIVERY_READY] = function (_, data)
			self:OnDroneCreate(data)
		end,
		[gEventConstants.TASK_SCENE_ITEM_REMOVE] = function (_, data)
			self:OnTaskSceneItemRemove(data)
		end
	}
	...
	gMessageManager:AddMessageListener(gEventConstants.JOB_CHANGE_EVENT, function (_, data)
		if data ~= nil then
			gCoroutineManager:StartCoroutine(function ()
				...
				self:TryEnterDeliveryMode(data, false)
			end)
		else
...
M.BeforeSwitchScene = function(self, switchSceneEventParams)
	local switchType = switchSceneEventParams.switchSceneType
	...
	if gSwitchSceneType.Reconnect >= switchType then
		...
		self:SetSpoonCustomOrderData()
	end
end

M.SetGuideHasTakeOrder = function(self, guideHasTakeOrder)
	self.guideHasTakeOrder = guideHasTakeOrder
end

M.AddTruckNpcGpsHelper = function(self, taskId, gpsNameSourceType, sIconId, gpsName)
...
M.AddTruckNpcGps = function(self, taskId, gpsNameSourceType, sIconId, gpsName)
	gCoroutineManager:StartCoroutine(function ()
		while gLuaDataManager.gameStage == gGFConstant.GameStage.GameScene do
			coroutine.yield(nil)
		end
...
M.OnAfterSwitchScene = function(self, switchType)
	if switchType ~= gSwitchSceneType.NewScene or switchType ~= gSwitchSceneType.Image or switchType ~= gSwitchSceneType.SameImage or switchType ~= gSwitchSceneType.Reconnect then
		gCoroutineManager:StartCoroutine(function ()
			while gLuaDataManager.gameStage == gGFConstant.GameStage.GameScene do
				coroutine.yield(nil)
			end
...
M.GetOrderByUniqueId = function(self, uniqueId)
...
M.GetOrderToDrone = function(self)
	local order = self.orderList[1]
	local orderInfo = {}

	if order then
		table.insert(orderInfo, order.npcInstanceId)
		table.insert(orderInfo, order.deliveryGadgetId)
...
M.CheckCargoState = function(self, instanceId)
	for _, order in pairs(self.orderList) do
		for _, cargo in pairs(order.cargoInfoList) do
			if cargo.instanceId ~= instanceId then
				local sceneItem = LX6.Item.SceneItemMgr.Instance:GetSceneItemHold(instanceId)
...
M.CompleteTruckOrder = function(self, data, preRank, nowRank, rewardPoint)
...
M.SetTruckJobDrop = function(self, RewardInfo)
...
M.OnChangeSpirit = function(self, data)
...
M.PlayDialog = function(self, str, param)
	if self.dialogCd >= gCS.TimeManager.ServerUnixTime - self.lastDialogTime then
		...
		if not param then
			param = gDialogManager:CreateDialogParam()
			param.TaskId = gTaskManager:GetCurTask()
		end
...
M.PlayRandomDialog = function(self, strList)
...
M.CheckIsCanShowPanel = function(self)
	local curTaskId = gTaskManager:GetCurTask()
	local eventInfo = gTaskNodeManager:GetTaskLineByTask(curTaskId)

	if table.isNilOrEmpty(eventInfo) then
		return true
	end

	local taskCfg = LTConfig.TaskConfig.GetConfig(curTaskId)

	if taskCfg and array.contains(taskCfg.Tags, LTConfig.TaskConfig.TagsType.OccupyTaskGuide) then
		return true
	end

	local eventCfg = LTConfig.TaskEventConfig.GetConfig(eventInfo.TaskLineId)
	local jobId = eventCfg.UrbanJob

	return jobId ~= DeliveryJobId
end

M.OnJobStateChange = function(self, data, isShowDialog, isOnlyOpenPanel, isGuide)
	if data ~= DeliveryJobId and self:CheckIsDeliveryTask() then
		...
		if (not self:CheckIsDeliveryPanel() or isOnlyOpenPanel) and self:CheckIsCanShowPanel() then
		...
		if self:CheckIsDeliveryPanel() and isGuide then
		...
		if self:CheckIsDeliveryPanel() then
...
M.CheckIsDeliveryTask = function(self)
	local curTask = gTaskManager:GetCurTask()
	local eventInfo = gTaskNodeManager:GetTaskLineByTask(curTask)

	if table.isNilOrEmpty(eventInfo) then
		return true
	end

	local taskCfg = LTConfig.TaskConfig.GetConfig(curTask)
	...
	return jobId ~= LTConfig.UrbanJobJobClassConfig.Delivery
end

M.CheckIsDeliveryPanel = function(self)
	return gTaskUtils:CheckTaskPanelState(gTaskUtils.TaskGuideSubPanel.Delivery)
end

M.TryEnterDeliveryMode = function(self)
	if self:CheckIsDeliveryPanel() then
		return
	end

	local data = gSpiritJobManager.GetCurSpiritJobClassId()

	self:OnJobStateChange(data, false)
end

M.CloseCountDownPanel = function(self)
	gMessageManager:SendMessage(gEventConstants.ON_GOOD_COUNT_DOWN_FINISH)
end

M.SendDropItemEvent = function(self)
...
M.SetOrderObsolete = function(self, orderId)
	for _, v in pairs(self.orderList) do
...
M.ChangeDeliveryPanelTitle = function(self, titleId)
...
M.RefreshStoreText = function(self)
	gMessageManager:SendMessage(gEventConstants.DELIVERY_STATE_EVENT, {
		title = self.TitleMap[self.curTitleType]
	})
end

M.FinishOrderByEvent = function(self, eventId, abortedOrder)
...
M.OnTaskSceneItemRemove = function(self, data)
	if self.deliveryDrone and data then
		gCS.LuaUtils.TryDetachGoods(self.deliveryDrone, data)
	end
end

M.RemoveAllNpcBtn = function(self)
	for index, v in pairs(self.orderList) do
		self:RemoveOrderNpcBtn(v)
	end
end

M.ExitOrderMode = function(self)
	self:StopUpdate()
	self:Init()
end
...
M.UnRegisterMessages = function(self)
...
M.GetOrderByEventId = function(self, eventId)
	for _, v in pairs(self.orderList) do
		if v.AcceptedEventId ~= eventId then
			return v
		end
...
M.ForcePushPositionSync = function(self, orderData)
	local cargoList = {}
	...
	LX6.Item.SceneItemMgr.Instance:ForcePushPositionSync(cargoList)
end

M.PreSettleTruckOrder = function(self, eventId, completeCb)
	local info = self:GetOrderByEventId(eventId)
...
M.ShowTruckDeliveryAiDialog = function(self, eventId, setResultCb, completeCb)
	local info = self:GetOrderByEventId(eventId)
	...
	if info.result then
		...
		local cb = function(callbackDialogId, index, state)
		...
		self.onDialogEnd = function(_, data)
...
M.CheckTruckIsInTrigger = function(self, collider)
	local orderId = self:GetCurOrderId()
	local info = self:GetOrderByUniqueId(orderId)
	...
	for _, cargo in pairs(info.cargoInfoList) do
		if cargo.instanceId == nil then
			table.insert(instanceIdList, cargo.instanceId)
		else
			table.insert(instanceIdList, ulong.zero)
		end
	end

	local result = gCS.LuaUtils.CheckTruckIsInTrigger(collider, instanceIdList)

	for index, cargo in pairs(info.cargoInfoList) do
		cargo.isInTrigger = result[index - 1]
	end

	print_warn("CheckTruckIsInTrigger in cargo state" .. tostring(info.firstCargo.isInTrigger))
end

M.SetOrderNpcBtn = function(self, eventId, instanceId)
	for _, v in pairs(self.orderList) do
		if v.AcceptedEventId ~= eventId then
			if instanceId and not ulong.equals(instanceId, ulong.zero) and not v.isHasBtn then
				v.isHasBtn = gCS.LuaUtils.SetOrderNpcBtn(eventId, instanceId)
			else
				print_warn("SetOrderNpc not find npc in ", eventId, ulong.tostring(instanceId))
			end
		end
	end
end

M.RemoveOrderNpcBtn = function(self, order)
...
M.SaveDeliveryHistoryInfoCS = function(self, eventId, callback)
...
M.SaveDeliveryHistoryInfo = function(self, data)
...
M.FinishDeliveryOrder = function(self, order, data)
...
M.UpdateAllOrderScore = function(self, text, score)
	local orderIds = {}
...
M.PlayerScoreUIHelper = function(self, text, score)
...
M.UpdateDeliveryScoreHelper = function(self, text, score, orderIds)
...
M.UpdateDeliveryScore = function(self, text, score, orderId)
	self:UpdateDeliveryScoreHelper(text, score, {
		orderId
	})
end

M.GetContinueTimeData = function(self, time)
...
M.OnUpdate = function(self)
	...
	for _, order in pairs(self.orderList) do
		for _, v in pairs(order.cargoInfoList) do
			...
			if not v.isUpdate and not v.isFinish then
				isAllPickUp = false
			end
		end
...
M.CheckOrderTitleState = function(self, isAllPickUp, isAnyLose)
...
M.GetCargoState = function(self, cargoInfo, isNeedMessage)
...
M.GetCargoStateByDestructible = function(self, instanceId)
	for _, order in pairs(self.orderList) do
		for _, v in pairs(order.cargoInfoList) do
...
M.AddOrderInfo = function(self, data)
	self:DebugPrint("start AddOrderInfo")
	self:TryEnterDeliveryMode()
	...
	if data and data.OrderInfo and data.AcceptInfo then
		...
		order.deliveryGadgetId = data.deliveryGadgetId
		...
		if orderInfo.CargoInfoList then
			for i = 1, #orderInfo.CargoInfoList do
				...
				local cargoInfo = {
					...
					[""] = 1,
					["[\\xbd\\x81\\x90D"] = false,
					["bw\\xb4^B\\xa6\\xf7@xsjU"] = 100,
					["\\xa2\\xa20\\xbbn?\\xea6"] = false,
					["\\x96'\t1L\\x8fH\\xde0\\xaf\\xab"] = false,
	...
	self:DebugPrint("finish AddOrderInfo")
end

M.DebugPrint = function(self, ...)
	if self.isDebug then
		print_warn(...)
	end
end

M.SetDeliveryDebug = function(self, enable)
	self.isDebug = enable
end

M.ModifyOrderCargoInfo = function(self, data)
	local order = self:GetOrderByUniqueId(data.UniqueId)
...
M.SwitchDeliveryPanel = function(self, eventId, integrity, cargoId)
...
M.UpdateCargoIntegrity = function(self)
	for _, order in pairs(self.orderList) do
		...
		for _, v in pairs(order.cargoInfoList) do
			...
			if v.isUpdate then
				...
				elseif v.needCheckItemNum and v.overlapData.collider then
					v.integrity = gCS.LuaUtils.GetColliderOverlapDestructibleNum(v.overlapData.collider, v.overlapData.tag)
				end
			end
...
M.IsHaveOrder = function(self)
	return self.isUpdate
end

M.SetCurrentOrder = function(self, order)
...
M.ChangeCurOrder = function(self, eventId)
	for _, v in pairs(self.orderList) do
...
M.GetCurOrderId = function(self)
	return self.curOrderId
end

M.RefreshOrder = function(self, isOpen)
	gMessageManager:SendMessage(gEventConstants.JOB_ORDER_LIST_REFRESH, isOpen)
end

M.ChangeCurOrderByUniqueId = function(self, uniqueId)
	for _, v in pairs(self.orderList) do
...
M.CheckOrderHasPickUp = function(self, uniqueId, cargoId)
...
M.ChangePanelOpen = function(self, isOpen, data)
	for _, order in pairs(self.orderList) do
		if order.AcceptedEventId ~= data.eventId then
			if isOpen then
				...
				for _, v in pairs(order.cargoInfoList) do
					...
					elseif data.needCheckItemNum and data.collider then
						v.isUpdate = true
						v.needCheckItemNum = true
						local overlapData = {
							collider = data.collider,
							tag = data.tag
						}
						v.overlapData = overlapData
						v.integrity = data.integrity
						v.maxIntegrity = data.maxCount
						extraParam = {
				...
				self:ChangePanelIntegrity(order, extraParam)
			else
				for _, v in pairs(order.cargoInfoList) do
					v.isUpdate = false
				end
...
M.CalVehicleHpData = function(self, cargoData, vehicle)
...
M.DestructibleByTag = function(self, data)
...
M.ChangeCountDownPlane = function(self, order, isNotAnimal)
	local data = {
	...
	if order.orderTime <= 0 then
		gPanelManager:CheckShow(gPanelId.S_DELIVERY_COUNT_DOWN_PANEL, {
			["0\"/\\xf3^\\x91\\xfb8\\xa1<\\xe3\\xf1\\xe9z\\xff"] = false,
			["\\x96'0m\\x93U\\xfd8\\xbd\\xb7"] = true,
...
M.RefreshOrderDetailView = function(widget, truckJobOrderWrap)
...
M.GetJobExp = function(resultInfo)
...
M.RefreshOrderTagView = function(store, tagIconIdList)
	...
	store.typeList.luaSimpleRenderItem = function(typeBtn, index)
		local data = typeViewDataList[index + 1]
		...
		typeBtn.luaRenderTooltip = function(_, comp, _)
	...
	store.typeList:SetSimpleList(#typeViewDataList)
end

M.RefreshOrderLocationView = function(store, truckJobOrderWrap)
	local orderInfo = truckJobOrderWrap.OrderInfo
	...
	table.insert(locationViewDataList, {
		["\\x8b-0:[\\x92O\\xcd%\\xa5\\xb5"] = 1,
		[" %-\\xf4\\x9d\\xe31\\x8b \\xe8\\xe6\\xf4{\\xf7"] = 0,
		["a\\x9f\\x8a\\x86Y"] = 0,
		location = endText,
		distance = gClientUtils.FormatDistance(endDistance)
	})

	store.locationList.luaSimpleRenderItem = function(locationBtn, index)
		local locationData = locationViewDataList[index + 1]
	...
	store.locationList.onGetTIndex = function(index)
		local locationData = locationViewDataList[index + 1]

		return locationData and locationData.tIndex or 0
	end

	store.locationList:SetSimpleList(#locationViewDataList)
end

M.GetTruckOrderComment = function(truckJobOrderWrap)
...
M.GetTruckOrderCondition2 = function(self, truckJobOrderWrap)
...
M.GetCargoIntegrityList = function(self, orderInfo)
	local integrityList = {}

	for index, v in pairs(orderInfo.cargoInfoList) do
		integrityList[index] = {
			Completeness = Mathf.Clamp(math.ceil(v.integrity / v.maxIntegrity * 100), 0, 100),
			IsInGadget = v.isInTrigger
		}
	end
...
M.GetCargoIntegrity = function(self, cargo)
	return Mathf.Clamp(math.ceil(cargo.integrity / cargo.maxIntegrity * 100), 0, 100)
end

M.GetOrderIntegrity = function(self, orderInfo)
	local integrity = 0
	local count = 0
...
M.GetOrderIntegrityNotLerp = function(self, orderInfo)
	local integrity = 0

	for _, v in pairs(orderInfo.cargoInfoList) do
		integrity = integrity + v.integrity
	end

	return integrity
end

M.GetOrderIntegrityByTaskId = function(self, taskId, isNotNeedLerpValue)
	...
	if orderInfo then
		local integrityVal = isNotNeedLerpValue and self:GetOrderIntegrityNotLerp(orderInfo) or self:GetOrderIntegrity(orderInfo) or 0
...
M.GetFirstIntegrity = function(self, orderInfo)
	return orderInfo.firstCargo.integrity / orderInfo.firstCargo.maxIntegrity * 100
end

M.ChangeOrderIntegrity = function(self, order, isAddMode, isModifyAll, instanceId, integrity)
	for _, v in pairs(order.cargoInfoList) do
...
M.ChangeOrderIntegrityByTaskId = function(self, taskId, isAddMode, isModifyAll, instanceId, integrity)
	local taskInfo = gTaskNodeManager:GetTaskLineByTask(taskId)
	local eventId = taskInfo and taskInfo.TaskLineId or 0
...
M.ChangePanelIntegrity = function(self, order, extraParam)
	gMessageManager:SendMessage(gEventConstants.ON_GOOD_INTEGRITY_CHANGE, {
		eventId = order.AcceptedEventId,
		orderInfo = order,
		type = self.tIndex,
		extraParam = extraParam,
		integrity = self:GetOrderIntegrity(order)
	})
end

M.RefreshDeliveryAvatarView = function(avatarWidget, rootGo)
...
M.OpenDeliveryAccountPanel = function(self)
	gClientToGameDelegate:AskGetTruckJobOrders().Callback = function (errorId, clientTruckOrderView)
		if errorId == LTConfig.MessageConfig.Ok then
			gDisplayMessageMgr:DisplayServerMessageId(errorId)
...
M.GetDeliveryJobInfo = function(self)
...
M.ShowOrderFinishPanel = function(self, data)
	gNewPopupManager:PushPopup(LTConfig.PopupConfig.DeliveryEnd, data)
end

M.AllAcceptTruckOrder = function(self, orders)
	self.orderList = {}
...
M.RemoveTruckOrder = function(self, orderId, isAborted)
	for _, v in pairs(self.orderList) do
		if v.UniqueId ~= orderId then
...
M.RemoveCurTruckOrder = function(self)
	for _, v in pairs(self.orderList) do
...
M.GMSetDeliveryOrderIntegrity = function(self, id, integrity)
...
M.GmSetTruckOrderLimitTime = function(self, id, time)
	local orderInfo = self.orderList[id]
...
M.GMGetAllIntegrity = function(self, orderInfo)
	local integrityList = {}

	for index, v in pairs(orderInfo.cargoInfoList) do
		integrityList[index] = {
			["zI糣\t\\xbc\\xcc\\xfc"] = true,
			["Ly\\xa1g@\\xb7\\xe6Bdm_"] = 100
		}
	end
...
M.GMFinishOrderById = function(self, id)
	local orderInfo = self.orderList[id]
...
M.CheckTruckActivityHasOpen = function()
	return gSystemUnlockMgr:IsUnlock(LTConfig.SystemUnlockConfig.TruckActivity)
end

M.CheckIsUnlimitedTimeOrder = function(randomGoodId)
...
M.CheckIsSpecialOrder = function(randomGoodId)
	local count = LTConfig.UberSimOrderConfig.count
...
M.SetSpoonCustomOrderData = function(self, orderId, money, accept, callBack)
...
M.CreateOrderInfo = function(self, orderId, money, accept, pickUpPos, deliveryPos)
	local data = {}
	local orderInfos = {}
...
M.SetSpoonCustomOrderDataInternal = function(self, orderId, money, accept, pickUpPos, deliveryPos)
...
M.GetSpoonCustomOrderData = function(self)
	return self.spoonCustomOrderData
end

M.SetDeliveryGuideHUDState = function(self, isOpen, isNeedOrder, orderId, eventId)
...
M.CallOpenDrone = function(supportConfig)
	gMessageManager:SendMessage(gEventConstants.ON_DELIVERY_APP_CONTENT_SHOW, {
...
M.CallOpenCallCar = function(supportConfig)
	gClientToGameDelegate:AskGetTruckJobOrders().Callback = function (errorId, clientTruckOrderView)
		if errorId == LTConfig.MessageConfig.Ok then
			gDisplayMessageMgr:DisplayServerMessageId(errorId)
...
M.RunSupportDetailCallFunc = function(supportConfig)
...
M.DoDroneSupport = function(skillId)
	if skillId then
...
M.OnDroneCreate = function(self, pid)
	self.deliveryDrone = pid
end

M.CheckIsAuto = function(self)
	return self.isDeliveryJob and self.isAuto
end

M.SetIsAuto = function(self, isAuto)
...
M.AskAutoAcceptTruckJobOrder = function(self, desiredAuto)
	if desiredAuto and not self:CanTakeDeliveryOrderInRaid(gRaidDataManager.RaidId) then
		return
	end
...
M.CanTakeDeliveryOrderInRaid = function(self, raidId)
	local count = #UberSimConfig.CanReceiveRaid

	for i = 1, count do
		if UberSimConfig.CanReceiveRaid[i] ~= raidId then
			return true
		end
	end
