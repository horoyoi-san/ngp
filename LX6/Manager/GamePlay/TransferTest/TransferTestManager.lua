-- Original chunk: @Lua\LuaFiles\LX6\Manager\GamePlay\TransferTest\TransferTestManager.lua
-- Decompiled from: 02182_TransferTestManager.lua_16ef2f98f7a9.luajit
...
M.StartPlaying = function(self, id, effectId, linkConfigData, successCondition, successCallback)
	self:OnInit()
...
M.GetLinkedOrderArray = function(self, linkConfigData)
...
M.OnGadgetStateBeginChange = function(_, params)
...
M.OnGadgetStateChange = function(_, entityInstanceId)
...
M.PlayLinkedGadgetEffect = function(self, id, effectId)
	...
	for _, line in ipairs(lineList) do
		if line[1] and line[2] and not gCS.LuaUtils.IsNull(goList[line[1]]) and not gCS.LuaUtils.IsNull(goList[line[1]]) then
			local uuid = gCS.EffectMgr:PlayEffectsOnTransform(effectId, LX6.Effect.EffectPlayTag.Gameplay, goList[line[1]].transform)
...
M.CheckSuccess = function(self, id)
...
M.StopPlaying = function(self, id)
	local data = self.InPlayingData[id]
