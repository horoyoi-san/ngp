-- Original chunk: @Lua\LuaFiles\LX6\Manager\State\UnitStateMgr.lua
-- Decompiled from: 00691_UnitStateMgr.lua_b5683eeebd42.luajit
...
M.SetInFightState = function(self, pid, value)
...
M.OnSelfEnterFightState = function(self)
...
M.OnSelfLeaveFightState = function(self)
...
M.OnEnterFightState = function(self, pid)
...
M.OnLeaveFightState = function(self, pid)
	local cs_unit = gCS.SceneDataMgr.GetUnit(pid)
...
M.OnLeaveClimbState = function(self, pid)
	local cs_unit = gCS.SceneDataMgr.GetUnit(pid)
...
M.LeaveClimbAction = function(self, cs_unit, force)
...
M.OnLeaveFlatten = function(self, pid)
	local cs_unit = gCS.SceneDataMgr.GetUnit(pid)
...
M.OnStateChangeNoInteractive = function(self, pid)
end

M.EnterOrLeaveFreeBuilding = function(self, enable)
...
M.ResetMyStateAndClearMove = function(self, forceChangeGroup, stopReason)
	...
	gCS.LogicStateMachineManager.ResetMyStateAndClearMove(gCS.MyPlayerManager.PlayerUnit, stopReason)
end

M.CSEnterOrLeaveSwing = function(self, enable)
	gUnitOperateManager.isJumpSwing = enable
	gPlayerManager.main.bindData.isSwing = enable
end

M.DoEnterIdleSpeed = function(self, rideStopAction, playPlayerStopAction, forcePlayIdleAction)
	local csunit = gCS.MyPlayerManager.PlayerUnit

	if gCS.BattleManager.openCSSkillRuntime and csunit and not gCS.LuaUtils.IsNull(csunit.PlayerObj) and gCS.SkillHelper:CheckDoEnterIdleSpeed(csunit.Pid) then
	...
	if not playPlayerStopAction and not rideStopAction then
		...
		if forcePlayIdleAction then
			...
			gCS.LogicStateMachineManager.DoEnterIdle(gCS.MyPlayerManager.PlayerUnit, forcePlayIdleAction)
		end
