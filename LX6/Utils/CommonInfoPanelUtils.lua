-- Original chunk: @Lua\LuaFiles\LX6\Utils\CommonInfoPanelUtils.lua
-- Decompiled from: 02245_CommonInfoPanelUtils.lua_c7220220ab1a.luajit

local CommonInfoPanelUtils = {
	["Ή!\\xed\\xea(\\xe2\\x87\\xfa\\x8b./"] = false
}
local this = CommonInfoPanelUtils

CommonInfoPanelUtils._AddCallback = function(origCallback, addCallback)
...
CommonInfoPanelUtils.SendPlayerGameplayInwardSignal = function(signalId)
	if signalId > 0 or not gCS.MyPlayerManager.PlayerUnit then
		return
	end

	gCS.LogicStateMachineManager.SendGameplayInwardSignal(gCS.MyPlayerManager.PlayerUnit, signalId)
end

CommonInfoPanelUtils.OpenCommonInfoPanel = function(data)
	...
	local openInwardSignalId = typeCfg.OpenInwardSignalId
	local closeInwardSignalId = typeCfg.CloseInwardSignalId
	...
	if openInwardSignalId <= 0 then
		data.onShowCallback = this._AddCallback(data.onShowCallback, function ()
			this.SendPlayerGameplayInwardSignal(openInwardSignalId)
		end)
	end

	if closeInwardSignalId <= 0 then
		data.onCloseCallback = this._AddCallback(data.onCloseCallback, function ()
			this.SendPlayerGameplayInwardSignal(closeInwardSignalId)
		end)
	end
...
CommonInfoPanelUtils.OpenCommonInfoPanelCs = function(self, data)
...
CommonInfoPanelUtils.OpenCommonInfoPanelWithoutTarget = function(data)
	data[1] = {
		id = data.id
	}
...
CommonInfoPanelUtils.OpenCommonInfoPanelWithoutTargetCs = function(self, args)
	local bookId = args[0]
	local onCloseCallback = args[1]

	local onCloseCallbackWrap = function()
...
CommonInfoPanelUtils.OnExitCloseUp = function()
...
CommonInfoPanelUtils.PlayPhoneAction = function()
...
CommonInfoPanelUtils.ExitPhoneAction = function()
...
CommonInfoPanelUtils.OnPanelClose = function()
