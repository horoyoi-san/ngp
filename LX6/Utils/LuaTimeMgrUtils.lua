-- Original chunk: @Lua\LuaFiles\LX6\Utils\LuaTimeMgrUtils.lua
-- Decompiled from: 00140_LuaTimeMgrUtils.lua_95ed7edec948.luajit
...
M.SetTimerCache = function(timer)
	if timer then
		timer.Stop(timer)
...
M.GetTimerCache = function(callback, delay, data, notAutoR, unscaled, isLogic)
	local timer = nil
	...
	if #M.TimerCache <= 0 then
		...
		timer.ResetTime(timer, delay, data)
...
M.NotDestroyDelay = function(callback, delay, loop, unscaled, isLogic)
	if delay <= 0 then
		local timer = M.GetTimerCache(callback, delay, nil, , unscaled, isLogic)
...
M.Delay = function(callback, delay, loop, unscaled, isLogic)
	if delay <= 0 then
		local timer = M.GetTimerCache(callback, delay, nil, , unscaled, isLogic)
		timer.loop = loop or 1
...
M.UnitDelay = function(pid, delay, callback, data, notAutoR, unscaled, isLogic, domust)
	if isLogic ~= nil then
		isLogic = true
	end
...
M.CancelUnitDelay = function(uuid)
...
M.CancelAllUnitDelay = function(pid)
	for i = #M.UnitDelayDictionary, 1, -1 do
