...
def log_and_check(api_url, name):
    ...
    log_dir = os.path.join(os.getcwd(), "无限大", "ngp", "log", name)
...
async def send_embed_message(channel_id, data, title, image_url=None):
    ...
    embed.set_footer(
        text="NGP Monitor • 无限大 BETA Tracker", icon_url=ANANTA_THUMBNAIL
    )
...
async def main():
    ...
    changed, _ = log_and_check(ANANTA_API, "无限大 BETA Launcher")
    ...
    if changed:
        ...
        if data:
            ...
            for channel_id in CHANNELS:
                ...
                await send_embed_message(channel_id, data, "无限大 BETA", ANANTA_IMAGE)
