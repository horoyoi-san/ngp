C_BigMapTooltip_GangsterSmallCamp = DefClass("C_BigMapTooltip_GangsterSmallCamp", C_BigMapTooltip_GangsterSmallCamp, C_BigMapTooltipBase)
local M = C_BigMapTooltip_GangsterSmallCamp
...
function M:SetUpInfo()
	if not self:ValidateTooltipInfo("gangsterSmallCampInfo") then
		return
	end

	self:GetStore("MapGangsterSmallCampTooltipStore")
	self:SetUpLocation()

	local info = self.tooltipInfo.gangsterSmallCampInfo
	...
	local scrollStore = gStoreManager:GetStoreGroup("MapGangsterSmallCampScrollStore"):GetStoreByWidget(self.store.scroll.content)
